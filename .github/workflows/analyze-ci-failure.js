...
function redactSensitiveData(value) {
    return String(value ?? '')
        ...
        .replace(/(["'](?:password|passwd|pwd|token|_?authToken|_?auth|accessToken|refreshToken|api[_-]?key|access[_-]?key|account[_-]?key|secret|client[_-]?secret|connection[_-]?string)["']\s*:\s*)(["'])(?:\\[^\r\n]|(?!\2)[^\\\r\n])*\2/gi, '$1$2[REDACTED]$2')
        ...
        .replace(/\b((?:[A-Za-z][A-Za-z0-9_.-]*[_-])?(?:password|passwd|pwd|token|api[_-]?key|access[_-]?key|account[_-]?key|secret|client[_-]?secret|sharedaccesskey|sharedaccesssignature|signature|private[_-]?key)|_?authToken|_?auth|accessToken|refreshToken)(\s*[:=]\s*)(["'])(?:\\[^\r\n]|(?!\3)[^\\\r\n])*\3/gi, '$1$2$3[REDACTED]$3')
        .replace(/\b((?:[A-Za-z][A-Za-z0-9_.-]*[_-])?(?:password|passwd|pwd|token|api[_-]?key|access[_-]?key|account[_-]?key|secret|client[_-]?secret|sharedaccesskey|sharedaccesssignature|signature|private[_-]?key)|_?authToken|_?auth|accessToken|refreshToken)(\s*[:=]\s*)(?!["'])([^;\r\n]+)/gi, '$1$2[REDACTED]');
}
