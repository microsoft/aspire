          ...
          run: |
            ...
            if [ "$VERDICT" = "code-issue" ]; then
              ...
              if [ "$RESOLVER_STATUS" -eq 0 ]; then
                # Canonicalization can reuse historical metadata from the memory branch.
                # Re-sanitize the resolved files before they are persisted or rendered.
              ...
              if git -C memory-repo diff --cached --quiet; then
                ...
                {
                  echo "publish_cause_issues=true"
                  echo "analyzed_at=${ANALYZED_AT}"
        ...
        - name: Publish cause issues
          ...
          with:
            script: |
              ...
              const analysisDirectory = process.env.ANALYSIS_DIR;
              ...
              await publisher.publishCauseIssues(github, context, core, {
                causesDirectory: path.join(analysisDirectory, 'causes'),
