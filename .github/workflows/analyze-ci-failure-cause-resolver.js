...
function resolveCauses({
    ...
    const currentIdentityByCanonicalId = new Map();
    ...
    for (const cause of causes) {
        ...
        const currentIdentity = {
            allAuthoritative:
                testNameMatch !== undefined ||
                proposedAlias !== undefined ||
                retryPatternMatch !== undefined ||
                explicitMatcherMatch !== undefined,
            testIdentity: normalizedCause.type === 'flaky-test' && normalizedCause.test_name
                ? normalizeTestName(normalizedCause.test_name)
                : undefined,
        };
        const existingIdentity = currentIdentityByCanonicalId.get(canonicalId);
        if (existing && !canMergeCurrentCauseIdentities(existingIdentity, currentIdentity)) {
            throw new Error(
                `Canonical cause '${canonicalId}' cannot merge current causes without ` +
                'independent authoritative identity evidence.');
        }
        normalizedById.set(
            canonicalId,
            existing ? mergeCurrentCauses(existing, normalizedCause) : normalizedCause);
        currentIdentityByCanonicalId.set(
            canonicalId,
            existingIdentity
                ? {
                    allAuthoritative:
                        existingIdentity.allAuthoritative && currentIdentity.allAuthoritative,
                    testIdentity:
                        existingIdentity.testIdentity === currentIdentity.testIdentity
                            ? existingIdentity.testIdentity
                            : undefined,
                }
                : currentIdentity);
    }
...
function canMergeCurrentCauseIdentities(existing, current) {
    if (existing.allAuthoritative && current.allAuthoritative) {
        return true;
    }

    return existing.testIdentity !== undefined &&
        existing.testIdentity === current.testIdentity;
}
...
function buildEvidence(cause, analysis, jobIds, jobNames, trustedJobLogs) {
    ...
    return [
        ...jobIds.map(jobId => trustedJobLogs?.[jobId]),
...
function normalizeTestName(testName) {
    ...
    // xUnit renders parameterized theory rows as "Method(parameter: value)".
    // Keep other parenthetical display-name qualifiers because they identify distinct tests.
    const isParameterizedTheory = argumentStart > 0 &&
        /^\(\s*[A-Za-z_][A-Za-z0-9_]*\s*:/.test(displayName.slice(argumentStart));
    const canonicalName = isParameterizedTheory ? displayName.slice(0, argumentStart) : displayName;
