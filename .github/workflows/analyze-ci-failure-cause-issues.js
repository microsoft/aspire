...
const CAUSE_LABEL_DEFINITIONS = new Map([
    [CAUSE_LABEL, {
        name: CAUSE_LABEL,
        color: 'f54219',
        description: '',
...
async function ensureCauseLabels(github, context, cause, ensuredLabels) {
    for (const labelName of labelsForCause(cause)) {
        if (ensuredLabels.has(labelName)) {
            continue;
        }
        const label = CAUSE_LABEL_DEFINITIONS.get(labelName);
        if (!label) {
            throw new Error(`Cause label '${labelName}' has no label definition.`);
        }
        await tracking.ensureLabel(
            github,
            context.repo.owner,
            context.repo.repo,
            label);
...
async function publishCauseIssue(
    ...
    await ensureCauseLabels(github, context, cause, ensuredLabels);
