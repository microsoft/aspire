...
The `debuggers` property lets you pass debug config specific to a language. Use `project` for C#/.NET services, `python` for Python, `java` for Java, and `apphost` for the apphost itself:
...
| Java | java | [Java Extension Pack](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-pack) |
| Go | delve | [Go](https://marketplace.visualstudio.com/items?itemName=golang.go) |
| Node.js | js-debug (built-in) | None |
| Bun | bun | [Bun for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=oven.bun-vscode) |
