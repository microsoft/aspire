...
The E2E fixtures target .NET 10, matching the SDK pinned by the repository's `global.json`. The Azure Functions shard additionally requires Azure Functions Core Tools v4 (`func`) on `PATH`. It installs the real .NET Install Tool, C#, Azure Resource Groups, and Azure Functions extensions into the isolated VS Code instance, generates a dedicated HTTPS certificate with shell-sensitive arguments, and activates the Azure Functions extension so it registers its `func` task definition and listeners. Aspire then creates and runs a registered `func: host start` task for the generated .NET isolated Functions resource; the shard probes its HTTPS endpoint and verifies that stopping the Aspire resource ends the same VS Code task. The separate `azure-functions-node` shard runs `azureFunctionsNode.e2e.test.ts`, which uses a dependency-free Node v3 HTTP Functions fixture and the source-built hosting integration to verify Core Tools startup, a real `pwa-node` inspector attachment, an HTTP-handler breakpoint with a matching source stack frame, and task/process/inspector cleanup. To run that proof, use `ASPIRE_EXTENSION_E2E_SHARD=azure-functions-node ASPIRE_EXTENSION_E2E_SPEC=out/test-e2e/test-e2e/azureFunctionsNode.e2e.test.js` with the same prerequisites and environment variables. CI runs both shards on Linux with pinned, checksum-verified copies of Core Tools 4.12.1, .NET Install Tool 3.1.0, C# 2.148.23 for Linux x64, Azure Resource Groups 0.12.7, and Azure Functions 1.22.0.
