...
ASPIRE_EXTENSION_E2E_ENABLE_CSHARP=true ASPIRE_EXTENSION_E2E_DOTNET_RUNTIME_VSIX=/path/to/vscode-dotnet-runtime.vsix ASPIRE_EXTENSION_E2E_CSHARP_VSIX=/path/to/csharp.vsix ASPIRE_EXTENSION_E2E_SHARD=debug-dashboard ASPIRE_EXTENSION_E2E_SPEC=out/test-e2e/test-e2e/debugDashboard.e2e.test.js ASPIRE_EXTENSION_E2E_CLI_PATH=/path/to/aspire corepack yarn test:e2e
...
The Linux debug dashboard shard installs the real .NET runtime and C# debugger extensions so C# AppHosts are launched through the extension rather than directly by the CLI. CI uses pinned, checksum-verified copies of .NET Install Tool 3.1.0 and C# 2.148.23 for Linux x64.
