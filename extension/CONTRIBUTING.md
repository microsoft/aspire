...
The resource-debugger shard is the only shard that gets the `AspireE2E.NodeApp` fixture and the `e2e-node` resource in the shared AppHost, so every other shard keeps the resource tree it asserts on today. It runs on the built-in js-debug adapter and needs nothing beyond `node` on `PATH`; a language whose debugger ships in a marketplace extension (Python, Go, Rust, MAUI) additionally needs that extension installed into the E2E VS Code instance, which only the Azure Functions shard does today. Because a skipped proof proves nothing, the runner fails this shard when it executes no tests, skips a test, or passes none - see `scripts/e2e-shard-results.js`.
