...
The Windows shard launches VS Code with `__COMPAT_LAYER=DetectorsAppHealth` to cover [Edge's compatibility-layer relaunch](https://github.com/microsoft/aspire/issues/20151). The extension explicitly unsets this variable only in Edge's browser-launch environment so js-debug can track the original browser process through managed breakpoint discovery.
...
The `winui-debug` shard is Windows-only. It installs pinned .NET Install Tool and C# VSIX files into the offline VS Code instance, generates a self-contained unpackaged WinUI project, and debugs it as an Aspire project resource. The fixture puts design-time intermediate outputs under `obj\design-time\` while keeping normal build outputs and NuGet restore assets at their standard paths. This separates the XAML compiler's JSON inputs, generated code, and saved state even when the C# language server reloads the project during Aspire's build. Waiting for initial project-load completion alone cannot prevent those later reloads from racing the real build. Before starting Aspire, the shard verifies that the C# definition provider resolves the XAML-generated `InitializeComponent` method from the isolated design-time directory. The app then writes a readiness marker from `Application.OnLaunched`; reaching that marker proves WinUI passed the `Application.Start` failure point covered by the regression. Dependabot does not update the raw Marketplace VSIX URLs or checksums. Treat them as regression inputs rather than current-version dependencies: rebaseline them manually only when VS Code compatibility requires it or the fixture is deliberately moved, and before changing the C# version verify that the old `TargetPath` launch still reproduces [the original crash](https://github.com/microsoft/aspire/issues/19091) while the `RunCommand` launch passes.
...
Diagnostics are left under `extension/.test-results`, `extension/.test-storage`, and `extension/.test-workspaces`, with shard-specific subdirectories when `ASPIRE_EXTENSION_E2E_SHARD` is set. The runner sets `ASPIRE_HOME` to an isolated per-run directory so settings-file failures do not touch or depend on the real user profile. Before cleanup it collects explicit diagnostic inputs: Aspire logs, `cache/apphost-info` metadata (excluding CLI leases), global `aspire.config.json`, VS Code logs/settings and screenshots, workspace `.aspire/logs`, current/legacy workspace settings, selected generated E2E project sources, and the WinUI readiness marker. It does not traverse Dashboard persistence, other Aspire-home caches/packages, workspace `.aspire/integrations` or generated SDK caches, or active Crashpad metrics. These diagnostic folders are ignored by git; `extension-e2e-tests.yml` uploads the prepared directories without a second file-selection allowlist.

Each source and file is attempted independently, and text is redacted before being written into the diagnostic folders. A copy or redaction failure remains a cleanup failure even if the tests passed, but does not prevent collecting the remaining diagnostics. Missing optional sources are allowed; files disappearing during traversal and unsupported file types such as symlinks are reported rather than silently ignored.

On Windows, a locked temporary-file cleanup warning also produces `windows-file-locks.json` in the shard's `.test-results/e2e` directory. A bounded PowerShell 7 helper uses read-only Restart Manager queries for the exact file under that run's root; it records resource-user PIDs, process creation identities, and application names without stopping them or collecting command lines/environment variables. Missing PowerShell, query errors, timeouts, and empty observations are reported explicitly. These point-in-time observations do not establish which handle blocked deletion, and do not suppress or reclassify the original cleanup warning.

If an observation names `dcp`, correlate its PID and creation time with the CLI logs before attributing it to an AppHost. The extension also runs `aspire doctor` during startup, and its connection probes are independent of debug sessions. Those probes monitor the owning doctor's PID and start identity so an extension-host reload that terminates doctor also triggers DCP shutdown, even when managed cleanup cannot run.
