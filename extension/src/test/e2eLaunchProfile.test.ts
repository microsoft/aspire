...
suite('E2E launch profile', () => {
    ...
    test('pins the C# debugger for the Linux debug dashboard shard', () => {
        const extensionRoot = path.resolve(__dirname, '..', '..');
        const runner = fs.readFileSync(path.join(extensionRoot, 'scripts', 'run-e2e.js'), 'utf8');
        const workflow = fs.readFileSync(path.join(extensionRoot, '..', '.github', 'workflows', 'extension-e2e-tests.yml'), 'utf8');
        const linuxDebugDashboardRow = /- name: Linux\r?\n\s+shardName: debug-dashboard[\s\S]*?(?=\r?\n\s+- name:)/.exec(workflow)?.[0];
        const csharpPrerequisitesIndex = workflow.indexOf('- name: Install C# debugger E2E prerequisites');
        const runStepIndex = workflow.indexOf('- name: Run extension E2E tests');
        const csharpInstallIndex = runner.indexOf('for (const csharpVsix of csharpVsixPaths)');
        const aspireInstallIndex = runner.indexOf("logStep('Installing VSIX')");

        assert.ok(linuxDebugDashboardRow);
        assert.ok(linuxDebugDashboardRow.includes('installCsharp: true'));
        assert.strictEqual(workflow.match(/installCsharp: true/g)?.length, 1);
        assert.ok(csharpPrerequisitesIndex >= 0);
        assert.ok(csharpPrerequisitesIndex < runStepIndex);
        assert.ok(workflow.includes('vscode-dotnet-runtime/3.1.0/vspackage'));
        assert.ok(workflow.includes('8e675ffe5f3674430d63e28d2dc05ab40f36c8494e9549e79d3995d721b13f5a'));
        assert.ok(workflow.includes('csharp/2.148.23/vspackage?targetPlatform=linux-x64'));
        assert.ok(workflow.includes('18b503e614a979212762683b35a4fa1806688ba773d5fe93bf62c9f9346db23f'));
        assert.ok(workflow.includes('ASPIRE_EXTENSION_E2E_ENABLE_CSHARP=true'));
        assert.ok(workflow.includes('ASPIRE_EXTENSION_E2E_DOTNET_RUNTIME_VSIX='));
        assert.ok(workflow.includes('ASPIRE_EXTENSION_E2E_CSHARP_VSIX='));
        assert.ok(runner.includes("const REQUIRED_CSHARP_EXTENSION_IDS = ['ms-dotnettools.vscode-dotnet-runtime', 'ms-dotnettools.csharp'];"));
        assert.ok(runner.includes("assertRequiredExtensionsRegistered(REQUIRED_CSHARP_EXTENSION_IDS, 'C# debugger')"));
        assert.ok(csharpInstallIndex >= 0);
        assert.ok(csharpInstallIndex < aspireInstallIndex);
    });
