...
suite('AspireDebugSession tests', () => {
    ...
    test('a default-discovery directory launch with multiple buildable AppHosts prompts and launches the selected one', async () => {
        const workspaceDir = makeTempDir();
        const firstAppHostPath = join(workspaceDir, 'ApiService', 'ApiService.AppHost.csproj');
        const secondAppHostPath = join(workspaceDir, 'WebApp', 'WebApp.AppHost.csproj');
        const workspaceFolder = createWorkspaceFolder('workspace', workspaceDir);
        ...
        const quickPickStub = sinon.stub(vscode.window, 'showQuickPick').callsFake(async (items: any) => {
            const resolvedItems = await items;
            return resolvedItems.find((item: any) => item.appHostPath === secondAppHostPath);
        });
        ...
        assert.strictEqual(spawnStub.firstCall.args[1], dirname(secondAppHostPath));
    });

    test('cancelling the AppHost picker aborts a multi-AppHost directory launch instead of failing non-interactively', async () => {
        const workspaceDir = makeTempDir();
        const firstAppHostPath = join(workspaceDir, 'ApiService', 'ApiService.AppHost.csproj');
        const secondAppHostPath = join(workspaceDir, 'WebApp', 'WebApp.AppHost.csproj');
        const workspaceFolder = createWorkspaceFolder('workspace', workspaceDir);
        ...
        // Give the (stubbed, synchronously-resolving) discovery and picker a turn of the event
        // loop to run, then confirm the CLI was never spawned for the ambiguous directory.
        await waitFor(() => (vscode.window.showQuickPick as sinon.SinonStub).calledOnce);
        await Promise.resolve();
        assert.strictEqual(spawnStub.called, false);
    });

    test('a directory launch with a single buildable AppHost does not prompt', async () => {
        const workspaceDir = makeTempDir();
        const onlyAppHostPath = join(workspaceDir, 'ApiService', 'ApiService.AppHost.csproj');
        const workspaceFolder = createWorkspaceFolder('workspace', workspaceDir);
        sinon.stub(vscode.workspace, 'getWorkspaceFolder').returns(workspaceFolder);

        const discoveryService = {
            discover: sinon.stub().resolves([
                { path: onlyAppHostPath, language: 'csharp', status: 'buildable' },
            ]),
        };
