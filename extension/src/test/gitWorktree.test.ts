...
suite('gitWorktree', () => {
    ...
    test('ensureIsolatedCliArg inserts --isolated before --', () => {
        assert.deepStrictEqual(ensureIsolatedCliArg(undefined, false), undefined);
        ...
        assert.deepStrictEqual(ensureIsolatedCliArg(['--isolated', '--no-build'], true), ['--isolated', '--no-build']);
