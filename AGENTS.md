...
- **reviewing-aspire-architecture**: Reviews an existing PR or diff for Aspire-specific architectural concerns. Use only when explicitly asked for deep architectural or pattern review, or for a concrete Aspire-domain question escalated by a generic reviewer; changed file paths alone do not qualify.
