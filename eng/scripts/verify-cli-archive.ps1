...
function Test-ArchiveSidecar {
    # Per-RID CLI archives must ship sidecar-free; each installer writes
    ...
    if ($strays) {
        ...
        throw "$ridFamily-* archive '$ArchiveFileName' must not contain '.aspire-install.json' (per-RID archives are shared across install sources; each installer authors its own sidecar after extraction). Found: $($strayPaths -join ', ')"
    }
