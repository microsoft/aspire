...
$normalizedVersion = if ([string]::IsNullOrWhiteSpace($Version)) {
    $embeddedVersions = @(
        $bundleDefinitions |
            ForEach-Object { Get-CurrentEmbeddedVersion $_ } |
            Sort-Object -Unique
    )
...
try {
    ...
    foreach ($preparedBundle in $preparedBundles) {
        ...
        # Keep shared hook provenance in the skills metadata, as before. Extensions are a separate
        # release asset and do not need a duplicate copy of that source-file metadata.
        if ($null -ne $hookMetadata -and $definition.AssetPrefix -eq 'aspire-skills') {
