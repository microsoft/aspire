...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    [InlineData(true)]
    [RequiresFeature(TestFeature.ContainerRuntime)]
    public Task VerifyWithInitFiles(bool useOracleScriptsLayout)
    {
        return VerifyInitializationFiles(useSetupBindMount: false, useOracleScriptsLayout: useOracleScriptsLayout);
    }
    ...
    {
        return VerifyInitializationFiles(useSetupBindMount: true, useOracleScriptsLayout: false);
    }

    private async Task VerifyInitializationFiles(bool useSetupBindMount, bool useOracleScriptsLayout)
    {
        // Creates a script that should be executed when the container is initialized.
        ...
        var scriptPath = useOracleScriptsLayout
            ? Directory.CreateDirectory(Path.Combine(initFilesPath, "startup")).FullName
            : initFilesPath;
        ...
        try
        {
            File.WriteAllText(Path.Combine(scriptPath, "01_init.sql"), $"""
                ALTER SESSION SET CONTAINER={oracleDbName};
                ALTER SESSION SET CURRENT_SCHEMA = SYSTEM;
                CREATE TABLE "Cars" ("Id" NUMBER(10) GENERATED BY DEFAULT ON NULL AS IDENTITY NOT NULL, "Brand" NVARCHAR2(2000) NOT NULL, CONSTRAINT "PK_Cars" PRIMARY KEY ("Id") );
                INSERT INTO "Cars" ("Id", "Brand") VALUES (1, 'BatMobile');
