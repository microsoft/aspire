...
public class PublishAsDockerfileTests(ITestOutputHelper outputHelper)
{
    ...
    [Fact]
    public async Task PublishAsContainerImageAfterDockerFileUsesPrebuiltImage()
    {
        var builder = TestDistributedApplicationBuilder.Create(DistributedApplicationOperation.Publish);
        ...
        Assert.Empty(container.Annotations.OfType<DockerfileBuildAnnotation>());
        Assert.False(executable.Resource.RequiresImageBuild());
        Assert.False(executable.Resource.RequiresImageBuildAndPush());

        var manifest = await ManifestUtils.GetManifest(executable.Resource, manifestDirectory: path).DefaultTimeout();
    ...
    [Fact]
    public async Task PublishAsDockerFileAfterContainerImageUsesDockerfile()
    {
        var builder = TestDistributedApplicationBuilder.Create(DistributedApplicationOperation.Publish);
        ...
        Assert.Single(container.Annotations.OfType<DockerfileBuildAnnotation>());
        Assert.True(executable.Resource.RequiresImageBuild());
        Assert.True(executable.Resource.RequiresImageBuildAndPush());
    ...
    [Fact]
    public async Task JavaScriptDefaultDockerfileFollowedByContainerImageUsesPrebuiltImage()
    {
        var builder = TestDistributedApplicationBuilder.Create(DistributedApplicationOperation.Publish);
        ...
        var frontend = builder.AddJavaScriptApp("frontend", path)
            .PublishAsContainerImage("contoso/frontend:1.0");

        builder.EvaluateContainerProjectionCallbacks();
        var container = Assert.IsAssignableFrom<ContainerResource>(frontend.Resource.AsContainer());

        Assert.Empty(container.Annotations.OfType<DockerfileBuildAnnotation>());
        Assert.False(frontend.Resource.RequiresImageBuild());
        Assert.False(frontend.Resource.RequiresImageBuildAndPush());

        var manifest = await ManifestUtils.GetManifest(frontend.Resource, manifestDirectory: path).DefaultTimeout();

        Assert.Equal("container.v0", manifest["type"]?.ToString());
        Assert.Equal("contoso/frontend:1.0", manifest["image"]?.ToString());
    }
...
#pragma warning restore ASPIREDOCKERFILEBUILDER001
    ...
    public async Task PublishProjectAsDockerFile()
    {
        ...
        var containerResource = GetContainerConfiguredOwner(builder, project.Resource);
    ...
    public void PublishProjectAsDockerFile_NoExistingEndpoints_DoesNotAddDefaultEndpoints()
    {
        ...
        var container = GetContainerConfiguredOwner(builder, project.Resource);
    ...
    public async Task PublishExecutableAsDockerFile_CalledMultipleTimes_PreservesExistingArgumentBehavior()
    {
        ...
        var containerResource = GetContainerConfiguredOwner(builder, executable.Resource);
    ...
    private static IResource GetContainerConfiguredOwner(
