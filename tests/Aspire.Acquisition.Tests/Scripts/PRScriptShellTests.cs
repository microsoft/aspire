...
public class PRScriptShellTests(ITestOutputHelper testOutput)
{
    ...
    // do not collide with the script install prefix (<prefix>/bin) or with other PR installs.
    ...
    // Under --dry-run the PR script must NOT write the source=pr sidecar
    ...
    // PR install prints the PATH-activation hint to stdout so users
    ...
    // The PR script must not mutate source sidecars under --dry-run.
