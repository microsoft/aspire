...
class DistributedApplicationBuilder:
    ...
    def add_connection_string_expression(self, name: str, connection_string_expression: ReferenceExpression, **kwargs: typing.Unpack["ConnectionStringResourceKwargs"]) -> ConnectionStringResource:  # type: ignore
        """Adds a connection string with a reference expression"""
        rpc_args: dict[str, typing.Any] = {'builder': self._handle}
        rpc_args['name'] = name
        rpc_args['connectionStringExpression'] = connection_string_expression
        result = self._client.invoke_capability(
...
class PipelineConfigurationContext:
    ...
    @model.setter
    def model(self, value: DistributedApplicationModel) -> None:
        """Sets the Model property"""
        self._client.invoke_capability(
            'Aspire.Hosting.Pipelines/PipelineConfigurationContext.setModel',
            {'context': self._handle, 'value': value}
        )
...
class AbstractResourceWithEnvironment(AbstractResource):
    ...
    @abc.abstractmethod
    def with_env(self, name: str, value: str | ReferenceExpression | EndpointReference | ParameterResource | AbstractResourceWithConnectionString | AbstractValueProvider) -> typing.Self:
...
class ContainerResourceKwargs(_BaseResourceKwargs, total=False):
    ...
    env: tuple[str, str | ReferenceExpression | EndpointReference | ParameterResource | AbstractResourceWithConnectionString | AbstractValueProvider]
...
class ContainerResource(_BaseResource, AbstractResourceWithEnvironment, AbstractResourceWithArgs, AbstractResourceWithEndpoints, AbstractResourceWithWaitSupport, AbstractResourceWithProbes, AbstractComputeResource):
    ...
    def __init__(self, handle: Handle, client: AspireClient, **kwargs: typing.Unpack[ContainerResourceKwargs]) -> None:
        ...
        if _env := kwargs.pop("env", None):
            if _validate_tuple_types(_env, (str, str | ReferenceExpression | EndpointReference | ParameterResource | AbstractResourceWithConnectionString | AbstractValueProvider)):
                rpc_args: dict[str, typing.Any] = {"builder": handle}
                rpc_args["name"] = typing.cast(tuple[str, str | ReferenceExpression | EndpointReference | ParameterResource | AbstractResourceWithConnectionString | AbstractValueProvider], _env)[0]
                rpc_args["value"] = typing.cast(tuple[str, str | ReferenceExpression | EndpointReference | ParameterResource | AbstractResourceWithConnectionString | AbstractValueProvider], _env)[1]
                handle = self._wrap_builder(client.invoke_capability('Aspire.Hosting/withEnvironment', rpc_args))
            else:
                raise TypeError("Invalid type for option 'env'. Expected: (str, str | ReferenceExpression | EndpointReference | ParameterResource | AbstractResourceWithConnectionString | AbstractValueProvider)")
