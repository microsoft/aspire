...
internal static class CliE2EAutomatorHelpers
{
        ...
        bool noBuild = false)
    {
        ...
        await auto.TypeAsync($"(set -o pipefail; ASPIRE_CLI_START_TIMEOUT={startupTimeoutSeconds.ToString(CultureInfo.InvariantCulture)} aspire start{isolatedFlag}{noBuildFlag} --format json | tee \"{jsonFile}\")");
        await auto.EnterAsync();
