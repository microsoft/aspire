...
/// <summary>
/// End-to-end tests for deploying Azure Container Registry resources via Aspire.
/// Tests the Aspire.Hosting.Azure.ContainerRegistry integration package.
/// </summary>
public sealed class AzureContainerRegistryDeploymentTests(ITestOutputHelper output)
{
    ...
    public async Task DeployAzureContainerRegistryResource()
    {
        ...
        await DeployAzureContainerRegistryResourceCore(cancellationToken);
    }

    private async Task DeployAzureContainerRegistryResourceCore(CancellationToken cancellationToken)
    {
        ...
        var resourceGroupName = DeploymentE2ETestHelpers.GenerateResourceGroupName("acr");

        output.WriteLine($"Test: {nameof(DeployAzureContainerRegistryResource)}");
