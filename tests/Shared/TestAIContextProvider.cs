...
public class TestAIContextProvider : IAIContextProvider, IAssistantDisplayContext
{
    ...
    public IceBreakersBuilder IceBreakersBuilder { get; } = new(new TestStringLocalizer<AIPrompts>());
    ...
    public async Task LaunchAssistantModelDialogAsync(
        ...
        bool openedForMobileView,
        string? returnFocusElementId)
    {
    ...
    private sealed class DisplayChangedSubscription(
        TestAIContextProvider provider,
        Func<Task> callback) : IDisposable
    {
        public void Dispose()
        {
            lock (provider._lock)
            {
                provider._displayChangedCallbacks.Remove(callback);
            }
        }
    }
}
