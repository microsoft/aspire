...
public class BrowserAutomationBuilderExtensionsTests(ITestOutputHelper testOutputHelper)
{
    ...
    public void WithBrowserAutomation_CreatesChildResource()
    {
        ...
        var browserResource = Assert.Single(appModel.Resources.OfType<BrowserResource>());
        Assert.Equal("web-browser", browserResource.Name);
        Assert.Equal(web.Resource.Name, browserResource.ParentResource.Name);
        Assert.Equal("chrome", browserResource.InitialConfiguration.Browser);
        Assert.Null(browserResource.InitialConfiguration.Profile);
        Assert.Contains(browserResource.Annotations.OfType<NameValidationPolicyAnnotation>(), static annotation => annotation == NameValidationPolicyAnnotation.None);

        Assert.True(browserResource.TryGetAnnotationsOfType<ResourceRelationshipAnnotation>(out var relationships));
        ...
        var command = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.OpenTrackedBrowserCommandName);
        ...
        Assert.True(command.Visibility.HasFlag(ResourceCommandVisibility.UI));
        Assert.True(command.Visibility.HasFlag(ResourceCommandVisibility.Api));
        var configureCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.ConfigureTrackedBrowserCommandName);
        ...
        Assert.True(configureCommand.Visibility.HasFlag(ResourceCommandVisibility.UI));
        Assert.True(configureCommand.Visibility.HasFlag(ResourceCommandVisibility.Api));
        var screenshotCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.CaptureScreenshotCommandName);
        ...
        Assert.True(screenshotCommand.Visibility.HasFlag(ResourceCommandVisibility.UI));
        Assert.True(screenshotCommand.Visibility.HasFlag(ResourceCommandVisibility.Api));
        var inspectCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.InspectBrowserCommandName);
        Assert.Equal(BrowserCommandStrings.InspectBrowserName, inspectCommand.DisplayName);
        Assert.Contains(inspectCommand.Arguments, input => input.Name == "maxElements" && input.InputType == InputType.Number && input.Required == false);
        Assert.Equal(ResourceCommandVisibility.Api, inspectCommand.Visibility);
        var getCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.GetCommandName);
        Assert.Contains(getCommand.Arguments, input => input.Name == "property" && input.InputType == InputType.Choice && input.Value == "text");
        var isCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.IsCommandName);
        Assert.Contains(isCommand.Arguments, input => input.Name == "state" && input.InputType == InputType.Choice && input.Value == "visible");
        var findCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.FindCommandName);
        Assert.Contains(findCommand.Arguments, input => input.Name == "kind" && input.InputType == InputType.Choice && input.Value == "text");
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.HighlightCommandName);
        var evaluateCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.EvaluateCommandName);
        Assert.Contains(evaluateCommand.Arguments, input => input.Name == "expression" && input.InputType == InputType.Text && input.Required);
        var cookiesCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.CookiesCommandName);
        Assert.Contains(cookiesCommand.Arguments, input => input.Name == "action" && input.InputType == InputType.Choice && input.Value == "get");
        var storageCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.StorageCommandName);
        Assert.Contains(storageCommand.Arguments, input => input.Name == "area" && input.InputType == InputType.Choice && input.Value == "local");
        var stateCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.StateCommandName);
        Assert.Contains(stateCommand.Arguments, input => input.Name == "clearExisting" && input.InputType == InputType.Boolean && !input.Required);
        var cdpCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.CdpCommandName);
        Assert.Contains(cdpCommand.Arguments, input => input.Name == "method" && input.InputType == InputType.Text && input.Required);
        Assert.Contains(cdpCommand.Arguments, input => input.Name == "session" && input.InputType == InputType.Choice && input.Value == "page");
        var tabsCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.TabsCommandName);
        Assert.Contains(tabsCommand.Arguments, input => input.Name == "action" && input.InputType == InputType.Choice && input.Value == "list");
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.FramesCommandName);
        var dialogCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.DialogCommandName);
        Assert.Contains(dialogCommand.Arguments, input => input.Name == "action" && input.InputType == InputType.Choice && input.Value == "accept");
        var downloadsCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.DownloadsCommandName);
        Assert.Contains(downloadsCommand.Arguments, input => input.Name == "behavior" && input.InputType == InputType.Choice && input.Value == "allow");
        var uploadCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.UploadCommandName);
        Assert.Contains(uploadCommand.Arguments, input => input.Name == "files" && input.InputType == InputType.Text && input.Required);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.BrowserUrlCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.BackBrowserCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.ForwardBrowserCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.ReloadBrowserCommandName);
        var clickCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.ClickBrowserCommandName);
        var selectorArgument = Assert.Single(clickCommand.Arguments, input => input.Name == "selector");
        Assert.Equal(InputType.Text, selectorArgument.InputType);
        Assert.True(selectorArgument.Required);
        Assert.Contains(clickCommand.Arguments, input => input.Name == "snapshotAfter" && input.InputType == InputType.Boolean && !input.Required);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.DoubleClickBrowserCommandName);
        var fillCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.FillBrowserCommandName);
        Assert.Contains(fillCommand.Arguments, input => input.Name == "value" && input.InputType == InputType.Text && input.Required);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.CheckBrowserCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.UncheckBrowserCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.NavigateBrowserCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.FocusBrowserElementCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.TypeBrowserTextCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.PressBrowserKeyCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.KeyDownBrowserCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.KeyUpBrowserCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.HoverBrowserElementCommandName);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.SelectBrowserOptionCommandName);
        var scrollCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.ScrollBrowserCommandName);
        Assert.Contains(scrollCommand.Arguments, input => input.Name == "deltaY" && input.InputType == InputType.Number && !input.Required);
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.ScrollIntoViewBrowserCommandName);
        var mouseCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.MouseBrowserCommandName);
        Assert.Contains(mouseCommand.Arguments, input => input.Name == "action" && input.InputType == InputType.Choice && input.Value == "move");
        Assert.Contains(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.WaitForBrowserCommandName);
        var waitCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.WaitCommandName);
        Assert.Contains(waitCommand.Arguments, input => input.Name == "urlContains" && input.InputType == InputType.Text && !input.Required);
        Assert.Contains(waitCommand.Arguments, input => input.Name == "loadState" && input.InputType == InputType.Choice && input.Value is null);
        Assert.Contains(waitCommand.Arguments, input => input.Name == "function" && input.InputType == InputType.Text && !input.Required);
        var waitUrlCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.WaitForBrowserUrlCommandName);
        Assert.Contains(waitUrlCommand.Arguments, input => input.Name == "match" && input.InputType == InputType.Choice && input.Value == "contains");
        var waitLoadStateCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.WaitForBrowserLoadStateCommandName);
        Assert.Contains(waitLoadStateCommand.Arguments, input => input.Name == "state" && input.InputType == InputType.Choice && input.Value == "load");
        var waitElementStateCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.WaitForBrowserElementStateCommandName);
        Assert.Contains(waitElementStateCommand.Arguments, input => input.Name == "state" && input.InputType == InputType.Choice && input.Value == "visible");
        var closeCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.CloseTrackedBrowserCommandName);
        Assert.True(closeCommand.Visibility.HasFlag(ResourceCommandVisibility.UI));
        Assert.True(closeCommand.Visibility.HasFlag(ResourceCommandVisibility.Api));

        var dashboardCommandNames = browserResource.Annotations.OfType<ResourceCommandAnnotation>()
            .Where(annotation => annotation.Visibility.HasFlag(ResourceCommandVisibility.UI))
        ...
        var snapshot = browserResource.Annotations.OfType<ResourceSnapshotAnnotation>().Single().InitialSnapshot;
    ...
    [Fact]
    public async Task WithBrowserAutomation_ConfigureCommandInputsDefaultToCurrentConfiguration()
    {
        using var builder = TestDistributedApplicationBuilder.Create(DistributedApplicationOperation.Run);
        ...
        web.WithBrowserAutomation(browser: "custom-browser", profile: "Profile 1", userDataMode: BrowserUserDataMode.Shared);

        using var app = builder.Build();
        var browserResource = Assert.Single(app.Services.GetRequiredService<DistributedApplicationModel>().Resources.OfType<BrowserResource>());
        var configureCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.ConfigureTrackedBrowserCommandName);
        var arguments = configureCommand.Arguments;
        var browserInput = arguments.Single(input => input.Name == BrowserConfigurationManager.BrowserInputName);
        var userDataModeInput = arguments.Single(input => input.Name == BrowserConfigurationManager.UserDataModeInputName);
        var profileInput = arguments.Single(input => input.Name == BrowserConfigurationManager.ProfileInputName);
    ...
    [Fact]
    public async Task WithBrowserAutomation_ConfigureCommandProfileInputDefaultsToCurrentConfigurationBeforeDependenciesLoad()
    {
        using var builder = TestDistributedApplicationBuilder.Create(DistributedApplicationOperation.Run);
    ...
    [Fact]
    public void WithBrowserAutomation_ConfigureCommandSaveInputDescribesUnavailableUserSecrets()
    {
        using var builder = TestDistributedApplicationBuilder.Create(DistributedApplicationOperation.Run);
        ...
        var configureCommand = Assert.Single(browserResource.Annotations.OfType<ResourceCommandAnnotation>(), annotation => annotation.Name == BrowserAutomationBuilderExtensions.ConfigureTrackedBrowserCommandName);
        var arguments = configureCommand.Arguments;
        var saveInput = arguments.Single(input => input.Name == BrowserConfigurationManager.SaveToUserSecretsInputName);
    ...
    {
        using var builder = TestDistributedApplicationBuilder.Create(DistributedApplicationOperation.Run);
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.BrowserConfigurationKey}"] = "msedge";
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.ProfileConfigurationKey}"] = "Default";
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.UserDataModeConfigurationKey}"] = nameof(BrowserUserDataMode.Shared);
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:web:{BrowserAutomationBuilderExtensions.BrowserConfigurationKey}"] = "chrome";
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:web:{BrowserAutomationBuilderExtensions.ProfileConfigurationKey}"] = "Profile 1";
        ...
        Assert.Equal("chrome", browserResource.InitialConfiguration.Browser);
        Assert.Equal("Profile 1", browserResource.InitialConfiguration.Profile);
    ...
    [Fact]
    public void WithBrowserAutomation_UsesBrowserConfiguration()
    {
        using var builder = TestDistributedApplicationBuilder.Create(DistributedApplicationOperation.Run);
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.LegacyBrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.BrowserConfigurationKey}"] = "msedge";
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.LegacyBrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.UserDataModeConfigurationKey}"] = nameof(BrowserUserDataMode.Shared);
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.LegacyBrowserConfigurationSectionName}:web:{BrowserAutomationBuilderExtensions.BrowserConfigurationKey}"] = "chrome";
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.LegacyBrowserConfigurationSectionName}:web:{BrowserAutomationBuilderExtensions.ProfileConfigurationKey}"] = "Profile 1";
    ...
    public void WithBrowserAutomation_UsesDetectedDefaultBrowserWhenConfigurationIsMissing()
    {
        ...
        Assert.Equal(BrowserConfiguration.GetDefaultBrowser(ChromiumBrowserResolver.TryResolveExecutable), browserResource.InitialConfiguration.Browser);
        Assert.Null(browserResource.InitialConfiguration.Profile);
    }
    ...
    {
        using var builder = TestDistributedApplicationBuilder.Create(DistributedApplicationOperation.Run);
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.BrowserConfigurationKey}"] = "chrome";
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.ProfileConfigurationKey}"] = "Profile 1";
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.UserDataModeConfigurationKey}"] = nameof(BrowserUserDataMode.Shared);
        ...
        Assert.Equal("msedge", browserResource.InitialConfiguration.Browser);
        Assert.Equal("Default", browserResource.InitialConfiguration.Profile);
        Assert.Equal(BrowserUserDataMode.Shared, browserResource.InitialConfiguration.UserDataMode);
    }
    ...
    {
        using var builder = TestDistributedApplicationBuilder.Create(DistributedApplicationOperation.Run);
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.UserDataModeConfigurationKey}"] = nameof(BrowserUserDataMode.Isolated);
    ...
    public async Task WithBrowserAutomation_CommandStartsTrackedSession()
    {
        ...
        var sessionManager = new FakeBrowserSessionManager();
        builder.Services.AddSingleton<IBrowserSessionManager>(sessionManager);
        ...
        var browserResource = app.Services.GetRequiredService<DistributedApplicationModel>().Resources.OfType<BrowserResource>().Single();
        var result = await app.ResourceCommands.ExecuteCommandAsync(browserResource, BrowserAutomationBuilderExtensions.OpenTrackedBrowserCommandName).DefaultTimeout();

        Assert.True(result.Success);

        var call = Assert.Single(sessionManager.Calls);
        Assert.Same(browserResource, call.Resource);
        Assert.Equal(browserResource.Name, call.ResourceName);
    ...
    public async Task WithBrowserAutomation_ConfigureCommandSavesResourceScopedBrowserSettingsAndAppliesImmediately()
    {
        ...
        var result = await app.ResourceCommands.ExecuteCommandAsync(browserResource, BrowserAutomationBuilderExtensions.ConfigureTrackedBrowserCommandName, arguments);

        Assert.True(result.Success);
        Assert.Equal("msedge", userSecretsManager.Secrets[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:web:{BrowserAutomationBuilderExtensions.BrowserConfigurationKey}"]);
        Assert.Equal(nameof(BrowserUserDataMode.Shared), userSecretsManager.Secrets[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:web:{BrowserAutomationBuilderExtensions.UserDataModeConfigurationKey}"]);
        Assert.Equal("Default", userSecretsManager.Secrets[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:web:{BrowserAutomationBuilderExtensions.ProfileConfigurationKey}"]);

        var effectiveConfiguration = browserResource.ResolveCurrentConfiguration(
            app.Services.GetRequiredService<IConfiguration>(),
            app.Services.GetRequiredService<BrowserConfigurationStore>());
    ...
    public async Task WithBrowserAutomation_ConfigureCommandSavesGlobalSettingsAndClearsProfile()
    {
        ...
        Assert.True(result.Success);
        Assert.Equal("chrome", userSecretsManager.Secrets[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.BrowserConfigurationKey}"]);
        Assert.Equal(nameof(BrowserUserDataMode.Isolated), userSecretsManager.Secrets[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.UserDataModeConfigurationKey}"]);
        Assert.Contains($"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.ProfileConfigurationKey}", userSecretsManager.DeletedSecrets);
    ...
    public async Task WithBrowserAutomation_ConfigureCommandDoesNotApplyRuntimeSettingsWhenUserSecretSaveFails()
    {
        ...
        var userDataModeKey = $"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:web:{BrowserAutomationBuilderExtensions.UserDataModeConfigurationKey}";
    ...
    [Fact]
    public async Task WithBrowserAutomation_ConfigureCommandRefreshesAllBrowserResourcesForGlobalSettings()
    {
        using var builder = TestDistributedApplicationBuilder.Create(testOutputHelper);
        var userSecretsManager = new RecordingUserSecretsManager();
        builder.Configuration[KnownConfigNames.VersionCheckDisabled] = "true";
        ...
        var browserResources = app.Services.GetRequiredService<DistributedApplicationModel>().Resources.OfType<BrowserResource>().ToArray();
        var webBrowserResource = browserResources.Single(resource => resource.ParentResource.Name == "web");
        var adminBrowserResource = browserResources.Single(resource => resource.ParentResource.Name == "admin");
        var arguments = CreateConfigureArguments("global", "chrome", nameof(BrowserUserDataMode.Isolated), "__aspire_browser_default__");
        var result = await app.ResourceCommands.ExecuteCommandAsync(webBrowserResource, BrowserAutomationBuilderExtensions.ConfigureTrackedBrowserCommandName, arguments);

        Assert.True(result.Success);

        await app.ResourceNotifications.WaitForResourceAsync(
            adminBrowserResource.Name,
            resourceEvent =>
    ...
    public async Task WithBrowserAutomation_CaptureScreenshotCommandReturnsArtifactResult()
    {
        ...
        var sessionManager = new FakeBrowserSessionManager
        {
            ScreenshotResult = new BrowserScreenshotCaptureResult(
                "session-0002",
                "msedge",
        ...
        var browserResource = app.Services.GetRequiredService<DistributedApplicationModel>().Resources.OfType<BrowserResource>().Single();
        var screenshotArguments = CreateArguments("""{"format":"jpeg","quality":80,"fullPage":true}""");
        var result = await app.ResourceCommands.ExecuteCommandAsync(browserResource, BrowserAutomationBuilderExtensions.CaptureScreenshotCommandName, screenshotArguments).DefaultTimeout();

        Assert.True(result.Success);
        Assert.Equal("web-browser", Assert.Single(sessionManager.CaptureScreenshotCalls));
        ...
        Assert.Equal("web-browser", document.RootElement.GetProperty("resourceName").GetString());
    ...
    public async Task WithBrowserAutomation_BrowserCommandsForwardArgumentsAndReturnJson()
    {
        ...
        await ExecuteBrowserCommandAsync(BrowserAutomationBuilderExtensions.CookiesCommandName, """{"action":"set","name":"empty","value":""}""");
        ...
        await ExecuteBrowserCommandAsync(BrowserAutomationBuilderExtensions.StorageCommandName, """{"area":"local","action":"set","key":"empty","value":""}""");
        ...
        Assert.Equal(
            [
                "GetAsync:web-browser:attr:#link:href",
                "IsAsync:web-browser:visible:#submit",
                "FindAsync:web-browser:role:button:Submit:1",
                "HighlightAsync:web-browser:#submit",
                "EvaluateAsync:web-browser:document.title",
                "CookiesAsync:web-browser:set:session:abc::/",
                "CookiesAsync:web-browser:set:empty:::",
                "StorageAsync:web-browser:local:set:theme:dark",
                "StorageAsync:web-browser:local:set:empty:",
                """StateAsync:web-browser:set:{"cookies":[],"localStorage":{},"sessionStorage":{}}:True""",
                """CdpAsync:web-browser:Runtime.evaluate:{"expression":"document.title","returnByValue":true}:page""",
                "TabsAsync:web-browser:open:https://example.com/:",
                "FramesAsync:web-browser",
                "DialogAsync:web-browser:accept:ok",
                "DownloadsAsync:web-browser:allow:/tmp/downloads:True",
                "UploadAsync:web-browser:#file:[\"/tmp/file.txt\"]",
                "GetUrlAsync:web-browser",
                "GoBackAsync:web-browser",
                "GoForwardAsync:web-browser",
                "ReloadAsync:web-browser",
                "ClickAsync:web-browser:#submit",
                "GetPageSnapshotAsync:web-browser:80:8000",
                "DoubleClickAsync:web-browser:#submit",
                "CheckAsync:web-browser:#accepted",
                "UncheckAsync:web-browser:#accepted",
                "FocusAsync:web-browser:#name",
                "TypeAsync:web-browser:#name:Aspire",
                "KeyDownAsync:web-browser:#name:Shift",
                "KeyUpAsync:web-browser:#name:Shift",
                "HoverAsync:web-browser:#submit",
                "ScrollAsync:web-browser:#panel:10:400",
                "ScrollIntoViewAsync:web-browser:#submit",
                "MouseAsync:web-browser:click:10:20:left:0:0",
                "WaitForUrlAsync:web-browser:/orders:contains:3000",
                "WaitForLoadStateAsync:web-browser:networkidle:3000",
                "WaitForElementStateAsync:web-browser:#submit:enabled:3000",
                "WaitForUrlAsync:web-browser:/dashboard:contains:3000",
                "WaitForFunctionAsync:web-browser:window.__appReady === true:3000"
            ],
        ...
        {
            var arguments = CreateArguments(json);
            var commandResult = await app.ResourceCommands.ExecuteCommandAsync(browserResource, commandName, arguments).DefaultTimeout();
    ...
    [Fact]
    public async Task WithBrowserAutomation_BrowserCommandValidationRejectsInvalidArgumentsBeforeExecution()
    {
        using var builder = TestDistributedApplicationBuilder.Create(testOutputHelper);
        var sessionManager = new FakeBrowserSessionManager();
        builder.Services.AddSingleton<IBrowserSessionManager>(sessionManager);
        ...
        var browserResource = app.Services.GetRequiredService<DistributedApplicationModel>().Resources.OfType<BrowserResource>().Single();
        var cases = new (string CommandName, string ArgumentsJson)[]
        {
            (BrowserAutomationBuilderExtensions.InspectBrowserCommandName, """{"maxElements":0}"""),
            (BrowserAutomationBuilderExtensions.GetCommandName, """{"property":"attr","selector":"#link"}"""),
            (BrowserAutomationBuilderExtensions.FindCommandName, """{"kind":"text","value":"Save","index":0}"""),
            (BrowserAutomationBuilderExtensions.CookiesCommandName, """{"action":"set","value":"abc"}"""),
            (BrowserAutomationBuilderExtensions.StorageCommandName, """{"area":"local","action":"set","value":"dark"}"""),
            (BrowserAutomationBuilderExtensions.StateCommandName, """{"action":"set","state":"[]"}"""),
            (BrowserAutomationBuilderExtensions.CdpCommandName, """{"method":"Runtime.evaluate","params":"[]"}"""),
            (BrowserAutomationBuilderExtensions.TabsCommandName, """{"action":"close"}"""),
            (BrowserAutomationBuilderExtensions.DownloadsCommandName, """{"behavior":"allow"}"""),
            (BrowserAutomationBuilderExtensions.UploadCommandName, """{"selector":"#file","files":"[]"}"""),
            (BrowserAutomationBuilderExtensions.NavigateBrowserCommandName, """{"url":"orders"}"""),
            (BrowserAutomationBuilderExtensions.ScrollBrowserCommandName, """{"deltaY":100001}"""),
            (BrowserAutomationBuilderExtensions.MouseBrowserCommandName, """{"action":"move","x":100001}"""),
            (BrowserAutomationBuilderExtensions.WaitForBrowserCommandName, "{}"),
            (BrowserAutomationBuilderExtensions.WaitCommandName, """{"elementState":"visible"}"""),
            (BrowserAutomationBuilderExtensions.WaitForBrowserUrlCommandName, """{"url":"/orders","timeoutMilliseconds":1}"""),
            (BrowserAutomationBuilderExtensions.CaptureScreenshotCommandName, """{"quality":101}""")
        };

        foreach (var (commandName, argumentsJson) in cases)
        {
            var result = await app.ResourceCommands.ExecuteCommandAsync(browserResource, commandName, CreateArguments(argumentsJson)).DefaultTimeout();

            Assert.False(result.Success);
            Assert.Equal("Command argument validation failed.", result.Message);
            var command = browserResource.Annotations.OfType<ResourceCommandAnnotation>().Single(annotation => annotation.Name == commandName);
            Assert.NotNull(command.ValidateArguments);
        }

        Assert.Empty(sessionManager.BrowserCommandCalls);
        Assert.Empty(sessionManager.CaptureScreenshotCalls);
    }
    ...
    public async Task WithBrowserAutomation_CaptureScreenshotCommandReturnsClearFailureWhenNoSessionIsActive()
    {
        ...
        var browserResource = app.Services.GetRequiredService<DistributedApplicationModel>().Resources.OfType<BrowserResource>().Single();
        var result = await app.ResourceCommands.ExecuteCommandAsync(browserResource, BrowserAutomationBuilderExtensions.CaptureScreenshotCommandName).DefaultTimeout();
    ...
    public async Task WithBrowserAutomation_CaptureScreenshotCommandWritesPngArtifact()
    {
        ...
        {
            using var builder = TestDistributedApplicationBuilder.Create(testOutputHelper);
            var sessionFactory = new FakeBrowserRunningSessionFactory();

            builder.Services.AddSingleton<IBrowserSessionManager>(sp =>
                new BrowserSessionManager(
                    ...
                    sp.GetRequiredService<ILogger<BrowserSessionManager>>(),
                    new BrowserArtifactWriter(sp.GetRequiredService<TimeProvider>(), () => artifactDirectory.FullName),
                    sessionFactory));
            ...
            var browserResource = app.Services.GetRequiredService<DistributedApplicationModel>().Resources.OfType<BrowserResource>().Single();
            var openResult = await app.ResourceCommands.ExecuteCommandAsync(browserResource, BrowserAutomationBuilderExtensions.OpenTrackedBrowserCommandName).DefaultTimeout();
            ...
            var logs = await ConsoleLoggingTestHelpers.WatchForLogsAsync(
                app.Services.GetRequiredService<ResourceLoggerService>().WatchAsync(browserResource.Name),
    ...
    public async Task WithBrowserAutomation_CommandUsesLatestConfiguredSettingsAndRefreshesProperties()
    {
        ...
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.BrowserConfigurationKey}"] = "msedge";
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.ProfileConfigurationKey}"] = null;
    ...
    public async Task WithBrowserAutomation_CommandRefreshesBrowserExecutablePropertyWhenRelaunchFails()
    {
        ...
        var firstResult = await app.ResourceCommands.ExecuteCommandAsync(browserResource, BrowserAutomationBuilderExtensions.OpenTrackedBrowserCommandName).DefaultTimeout();
        ...
        try
        {
            ...
            builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.BrowserConfigurationKey}"] = tempBrowserPath;
            sessionFactory.NextStartException = new InvalidOperationException("Launch failed.");

            var secondResult = await app.ResourceCommands.ExecuteCommandAsync(browserResource, BrowserAutomationBuilderExtensions.OpenTrackedBrowserCommandName).DefaultTimeout();
    ...
    public async Task WithBrowserAutomation_CommandRemovesStaleBrowserExecutablePropertyWhenBrowserCannotBeResolved()
    {
        ...
        builder.Configuration[$"{BrowserAutomationBuilderExtensions.BrowserConfigurationSectionName}:{BrowserAutomationBuilderExtensions.BrowserConfigurationKey}"] = "missing-browser";
        sessionFactory.NextStartException = new InvalidOperationException("Launch failed.");
    ...
    public async Task WithBrowserAutomation_CommandClearsLastErrorAfterSuccessfulLaunch()
    {
        ...
        var browserResource = app.Services.GetRequiredService<DistributedApplicationModel>().Resources.OfType<BrowserResource>().Single();
        var failedResult = await app.ResourceCommands.ExecuteCommandAsync(browserResource, BrowserAutomationBuilderExtensions.OpenTrackedBrowserCommandName).DefaultTimeout();
        ...
        var successfulResult = await app.ResourceCommands.ExecuteCommandAsync(browserResource, BrowserAutomationBuilderExtensions.OpenTrackedBrowserCommandName).DefaultTimeout();
    ...
    public async Task WithBrowserAutomation_CommandFailsWhenEndpointIsMissing()
    {
        ...
        Assert.False(result.Success);
        Assert.Equal("Resource 'web' does not have an HTTP or HTTPS endpoint. Browser requires an endpoint to navigate to.", result.Message);
    ...
    public async Task WithBrowserAutomation_PreservesLastErrorWhenOneOfMultipleSessionsFails()
    {
        ...
        Assert.True((await app.ResourceCommands.ExecuteCommandAsync(browserResource, BrowserAutomationBuilderExtensions.OpenTrackedBrowserCommandName).DefaultTimeout()).Success);
        Assert.True((await app.ResourceCommands.ExecuteCommandAsync(browserResource, BrowserAutomationBuilderExtensions.OpenTrackedBrowserCommandName).DefaultTimeout()).Success);
    ...
    public async Task BrowserEventLogger_LogsSuccessfulNetworkRequests()
    {
        ...
        var resourceLogger = resourceLoggerService.GetLogger("web-browser");
        var eventLogger = new BrowserEventLogger("session-0001", resourceLogger);
        var logs = await CaptureLogsAsync(resourceLoggerService, "web-browser", () =>
        {
    ...
    private static BrowserCdpProtocolEvent ParseProtocolEvent(string json)
    {
        var payload = Encoding.UTF8.GetBytes(json);
        return BrowserCdpProtocol.ParseEvent(BrowserCdpProtocol.ParseMessageHeader(payload), payload)
            ?? throw new InvalidOperationException("Expected a browser protocol event frame.");
    }

    private static InteractionInputCollection CreateArguments(string json)
    {
        using var document = JsonDocument.Parse(json);
        var root = document.RootElement;
        if (root.ValueKind != JsonValueKind.Object)
        {
            throw new InvalidOperationException("Expected command arguments to be a JSON object.");
        }
    ...
    private sealed class FakeBrowserSessionManager : IBrowserSessionManager
    {
        ...
        public BrowserScreenshotCaptureResult ScreenshotResult { get; set; } = new(
            "session-0001",
            "chrome",
            "/fake/browser",
        ...
        public Task StartSessionAsync(BrowserResource resource, BrowserConfiguration configuration, string resourceName, Uri url, CancellationToken cancellationToken)
        {
            Calls.Add(new SessionStartCall(resource, configuration, resourceName, url));
            return Task.CompletedTask;
        }

        public Task<BrowserScreenshotCaptureResult> CaptureScreenshotAsync(string resourceName, BrowserScreenshotCaptureOptions options, CancellationToken cancellationToken)
        {
        ...
        public Task<string> NavigateAsync(BrowserResource resource, string resourceName, Uri url, CancellationToken cancellationToken)
        {
    ...
    private sealed record SessionStartCall(BrowserResource Resource, BrowserConfiguration Configuration, string ResourceName, Uri Url);

    private sealed class FakeBrowserRunningSessionFactory : IBrowserRunningSessionFactory
    {
        public List<FakeBrowserRunningSession> Sessions { get; } = [];
            ...
            CancellationToken cancellationToken)
        {
            ...
            return Task.FromResult<IBrowserRunningSession>(session);
        }
    }

    private sealed class FakeBrowserRunningSession(
        string sessionId,
        string browserExecutable,
        int? processId,
        int sessionNumber,
        BrowserHostOwnership browserHostOwnership,
        DateTime startedAt) : IBrowserRunningSession
    {
