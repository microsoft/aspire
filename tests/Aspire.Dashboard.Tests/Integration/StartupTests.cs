...
public class StartupTests(ITestOutputHelper testOutputHelper)
{
    ...
    public async Task EndPointAccessors_AppStarted_BrowserGet_Success()
    {
        ...
        // Assert
        response.EnsureSuccessStatusCode();
        var contentSecurityPolicyValues = response.Headers.GetValues(HeaderNames.ContentSecurityPolicy).ToList();
        Assert.NotEmpty(contentSecurityPolicyValues);
        Assert.All(contentSecurityPolicyValues, Assert.NotEmpty);
    }
