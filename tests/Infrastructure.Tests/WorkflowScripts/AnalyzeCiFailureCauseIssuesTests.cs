...
public sealed class AnalyzeCiFailureCauseIssuesTests : IDisposable
{
    ...
    [Theory]
    [InlineData("infra-failure", "ci-failure-cause")]
    [InlineData("flaky-test", "ci-failure-cause,test-failure")]
    [InlineData("main-repository-breakage", "ci-failure-cause,main-ci-break")]
    [RequiresTools(["node"])]
    public async Task PublishEnsuresEveryCauseLabelBeforeIssueLookup(
        string causeType,
        string expectedLabelNames)
    {
        ...
        var expectedNames = expectedLabelNames.Split(',');
        Assert.Equal(expectedNames, result.LabelRequests.Select(request => request.Name));
        Assert.Equal(expectedNames.Length, result.Calls.Count(call => call == "createLabel"));
        Assert.True(
            Array.LastIndexOf(result.Calls, "createLabel") < Array.IndexOf(result.Calls, "listIssues"));

        foreach (var request in result.LabelRequests)
        {
            var expected = request.Name switch
            {
                "ci-failure-cause" => new LabelRequest("ci-failure-cause", "f54219", ""),
                "test-failure" => new LabelRequest("test-failure", "ededed", ""),
                "main-ci-break" => new LabelRequest(
                    "main-ci-break",
                    "b60205",
                    "Deterministic repository breakage on the main branch"),
                _ => throw new InvalidOperationException($"Unexpected label '{request.Name}'.")
            };
    ...
    [Fact]
    [RequiresTools(["node"])]
    public async Task PublishAcceptsExistingCauseLabelsWithoutUpdatingMetadata()
    {
        var result = await InvokeHarnessAsync<PublishResult>(
            "publishCauseIssues",
            new
            {
                ...
                existingLabels = new[] { "ci-failure-cause", "test-failure" },
                issues = Array.Empty<object>()
            });

        Assert.Equal(
            ["ci-failure-cause", "test-failure"],
            result.LabelRequests.Select(request => request.Name));
    ...
    [InlineData(true)]
    [RequiresTools(["node"])]
    public async Task PublishMovesOccurrenceToSelectedCanonicalBeforeClosingPreviousIssue(bool includePriorReceipt)
    {
        var priorOccurrence = includePriorReceipt
            ? new
            {
                run_id = 991,
                observed_at = "2026-08-29T18:30:00Z",
                issue_published = true,
                issue_published_url = "https://github.com/microsoft/aspire/issues/10"
            }
            : (object)new
            {
                run_id = 991,
        ...
        Assert.Contains("[100](https://github.com/microsoft/aspire/actions/runs/100)", canonical.Body);
        Assert.Contains("[900](https://github.com/microsoft/aspire/actions/runs/900)", canonical.Body);
        Assert.Contains("[991](https://github.com/microsoft/aspire/actions/runs/991)", canonical.Body);
        Assert.Contains("Showing 3 most recent of 3 occurrences.", canonical.Body);
        Assert.Equal("open", canonical.State);

        var previous = Assert.Single(result.Issues, issue => issue.Number == 10);
        Assert.Equal("closed", previous.State);

        var occurrence = Assert.Single(
            result.StoredCause.GetProperty("occurrences").EnumerateArray(),
            occurrence => occurrence.GetProperty("run_id").GetInt32() == 991);
                ...
                storedCause = new
                {
                    ...
                    occurrences = new object[]
                    {
                        ...
                        new
                        {
                            ...
                            observed_at = "2026-08-29T18:30:00Z",
                            issue_published = true,
                            issue_published_url = "https://github.com/microsoft/aspire/issues/20"
                        },
                    }
                },
                storedAliases = new[]
    ...
    [Fact]
    [RequiresTools(["node"])]
    public async Task PublishDoesNotCloseDuplicateWithUnsupportedOccurrenceHistory()
    {
        var result = await InvokeHarnessAsync<PublishResult>(
            "publishCauseIssues",
            new
    ...
    [Fact]
    [RequiresTools(["node"])]
    public async Task PublishDoesNotCloseDuplicateWithConflictingOccurrenceHistory()
    {
        var result = await InvokeHarnessAsync<PublishResult>(
            "publishCauseIssues",
            new
                ...
                issues = new object[]
                {
                    ...
                    new
                    {
                        ...
                        body = """
                            ...
                            | Date | Build | Job | Context |
                            |------|-------|-----|----|
                            | 2026-08-27 | [100](https://github.com/microsoft/aspire/actions/runs/100) | ` Tests / Windows ` | #19804 |
                            <!-- ci-failure-occurrences:end -->
                            """
                    }
                }
        ...
        Assert.Contains(
            result.Warnings,
            warning => warning.Contains(
                "conflicting duplicate occurrence history",
                StringComparison.Ordinal));
    }
    ...
    private sealed record LabelRequest(string Name, string Color, string Description);
