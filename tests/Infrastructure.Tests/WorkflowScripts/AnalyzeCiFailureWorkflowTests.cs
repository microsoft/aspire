...
public sealed class AnalyzeCiFailureWorkflowTests(ITestOutputHelper output) : IDisposable
{
    ...
    public async Task AnalysisValidatorAllowsReusedFlakyCauseForParameterizedTheoryRows()
    {
        ...
        await WriteValidationFixtureAsync(
            $$"""{"run_id":123,"run_scope":"pull-request","verdict":"flaky-test","pr":{"number":42},"failed_jobs":[{"id":123,"classification":"flaky-test"}],"failed_tests":[{"name":"{{testName}}","job":"Tests","error":"boom","stack_trace":"frame","classification":"flaky","reason":"Intermittent"}],"causes":["flaky-failure"]}""",
            """{"run_id":123,"run_scope":"pull-request","pr_numbers":"42"}""",
            """[{"id":123,"name":"Tests"}]""",
            "flaky-failure.json",
            $$"""{"id":"flaky-failure","type":"flaky-test","title":"Flaky failure","test_name":"{{testName}}","error_pattern":"boom","job_ids":[123]}""");
        var priorCausesDirectory = Directory.CreateDirectory(
            Path.Combine(_workspace.Path, "ci-failure-data", "prior-causes")).FullName;
        await File.WriteAllTextAsync(
            Path.Combine(priorCausesDirectory, "flaky-failure.json"),
            """{"id":"flaky-failure","type":"flaky-test","title":"Stored failure","test_name":"Aspire.Sample.Tests.TheoryTests.Sample(value: secondary)","error_pattern":"old"}""");
    ...
    public void AnalysisConsumersReceiveDownloadedAnalysisDirectory()
    {
        ForEachExecutableWorkflow(workflow =>
        {
            ...
            Assert.DoesNotContain("path.dirname(outputFile)", workflow, StringComparison.Ordinal);
            ...
            Assert.Equal(5, analysisDirAssignments.Length);
            Assert.All(
