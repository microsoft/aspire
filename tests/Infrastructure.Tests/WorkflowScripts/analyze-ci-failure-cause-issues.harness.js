        ...
        rest: {
            issues: {
                ...
                createLabel: async ({ name, color, description }) => {
                    ...
                    store.labelRequests.push({ name, color, description });
                    if (store.existingLabels.has(name)) {
                        const error = new Error(`Label '${name}' already exists.`);
                        error.status = 422;
                        throw error;
                    }
                    return { data: {} };
...
async function dispatch(operation, payload) {
    ...
    const store = {
        next: 1000,
        created: false,
        existingLabels: new Set(payload.existingLabels ?? []),
        labelRequests: [],
    ...
    const options = {
        ...
        prNumber: payload.prNumber ?? (payload.runScope === 'main' ? 0 : 19804),
    ...
    return {
        ...
        publishes,
        calls: github.calls,
        labelRequests: store.labelRequests,
        warnings,
