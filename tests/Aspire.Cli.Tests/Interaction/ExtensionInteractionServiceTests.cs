...
public class ExtensionInteractionServiceTests(ITestOutputHelper outputHelper)
{
    ...
    [Fact]
    public async Task DisplayErrorWithLogFile_WhenSupported_UsesSingleBackchannelError()
    {
        using var workspace = TemporaryWorkspace.CreateForCli(outputHelper);
        var logFilePath = Path.Combine(workspace.WorkspaceRoot.FullName, "aspire.cli.log");
        const string errorMessage = "The project could not be built.";
        var displayedErrors = new List<(string ErrorMessage, string LogFilePath)>();
        var legacyErrors = new List<string>();
        var displayedMessages = new List<(string EmojiName, string Message)>();
        var backchannel = new TestExtensionBackchannel
        {
            HasCapabilityAsyncCallback = (capability, _) => Task.FromResult(capability == KnownCapabilities.ErrorNotificationLogAction),
            DisplayErrorWithLogFileAsyncCallback = (error, path) =>
            {
                displayedErrors.Add((error, path));
                return Task.CompletedTask;
            },
            DisplayErrorAsyncCallback = error =>
            {
                legacyErrors.Add(error);
                return Task.CompletedTask;
            },
            DisplayMessageAsyncCallback = (emoji, message) =>
            {
        ...
        interactionService.DisplayErrorWithLogFile(errorMessage, logFilePath);
        await interactionService.FlushAsync();

        Assert.Equal([(errorMessage, logFilePath)], displayedErrors);
        Assert.Empty(legacyErrors);
        Assert.Empty(displayedMessages);
    }

    [Fact]
    public async Task DisplayErrorWithLogFile_WhenUnsupported_UsesLegacyBackchannelMessages()
    {
        using var workspace = TemporaryWorkspace.CreateForCli(outputHelper);
        var logFilePath = Path.Combine(workspace.WorkspaceRoot.FullName, "aspire.cli.log");
        const string errorMessage = "The project could not be built.";
        ...
        var displayedMessages = new List<(string EmojiName, string Message)>();
        var combinedErrors = new List<(string ErrorMessage, string LogFilePath)>();
        var backchannel = new TestExtensionBackchannel
        {
        ...
        Assert.Equal([errorMessage], displayedErrors);
        Assert.Empty(combinedErrors);
        var displayedMessage = Assert.Single(displayedMessages);
        Assert.Equal(KnownEmojis.PageFacingUp.Name, displayedMessage.EmojiName);
        Assert.Equal(
            string.Format(CultureInfo.CurrentCulture, InteractionServiceStrings.SeeLogsAt, logFilePath),
