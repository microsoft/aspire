...
public class AspireSkillsBundleTests
{
    ...
    [Theory]
    // Fix 2/5: defense-in-depth bundle skill-name validation.
    // (Empty/whitespace names are rejected earlier with a different "missing name" error and
    // covered by a separate code path, so they are excluded from this safety-validator theory.)
    [InlineData("../aspire")]              // path traversal
    [InlineData("aspire/foo")]             // forward slash
    [InlineData("aspire\\foo")]            // backslash
    [InlineData(".")]                      // current dir
    [InlineData("..")]                     // parent dir
    [InlineData("foo\u0001bar")]           // C0 control (U+0001); rejected only by char.IsControl on POSIX
    [InlineData("foo\u0085bar")]           // NEL (U+0085, C1 control)
    [InlineData("CON")]                    // Windows reserved device name
    [InlineData("CON.md")]                 // reserved name with extension is still reserved
    [InlineData("nul")]                    // case-insensitive
    [InlineData("LPT3")]                   // reserved device with number
    // U+00E9 expressed as e + combining acute (NFD). NFC would yield "café"; the bundle
    // name must already be NFC so case-insensitive filesystems don't collide with a
    // different on-disk name.
    [InlineData("cafe\u0301")]
    public async Task LoadAsync_ThrowsWhenSkillNameIsUnsafe(string unsafeName)
    {
        var bundleDirectory = CreateTempDirectory();
        ...
        try
        {
            ...
            Assert.Contains("skill name", exception.Message, StringComparison.OrdinalIgnoreCase);
    ...
    [Fact]
    public async Task LoadAsync_ThrowsWhenSkillNameIsNotSafe()
    {
        var bundleDirectory = CreateTempDirectory();
