...
public class PeerInstallProbeTests(ITestOutputHelper outputHelper) : IDisposable
{
    ...
    public async Task ProbeAsync_InvokesPeerWithInfoSelfFormatJson()
    {
        ...
        // `aspire --info` would run full installation discovery and the peer would
    ...
    [Fact]
    public async Task ProbeAsync_PeerEmitsBareInstallationArray_ReturnsOk()
    {
        // `aspire --info --self --format json` writes a bare array of
        // InstallationInfo rows (see src/Aspire.Cli/Commands/InfoOptionAction.cs).
        // The probe must accept that shape — not only the wrapped object form —
        // so it stays compatible with new peers.
        using var fakePeer = FakePeerScript.Build(
            outputHelper,
            stdout: """
                    [
        ...
        var ok = AssertProbeOk(result);
        Assert.Equal("13.0.0", ok.Info.Version);
        Assert.Equal("pr-17400", ok.Info.Channel);
        Assert.Equal("pr", ok.Info.Source);
    ...
    {
        // Empty rich output is treated as "--info didn't tell us anything useful"
    ...
    {
        // Invalid JSON on the --info path is treated as a peer failure mode
...
internal static class FakePeerScript
{
    ...
    /// <c>--info --self --format json</c> invocation and the <c>--version</c>
    /// fallback.
    /// </summary>
...
internal abstract record ScriptBody
{
    ...
    private sealed record DoctorOrVersionScript(string DoctorStdout, int DoctorExitCode, string VersionStdout, int VersionExitCode) : ScriptBody
    {
        ...
        public override string RenderBatch()
        {
            var sb = new System.Text.StringBuilder();
            sb.AppendLine("@echo off");
            sb.AppendLine("if \"%~1\" == \"--info\" goto :doctor");
    ...
    {
        // Minimal valid --info JSON: enough for the probe to take the primary
