...
public sealed class TypeScriptAppHostToolchainResolverTests(ITestOutputHelper outputHelper)
{
    ...
    [Fact]
    public void Resolve_WhenGrandparentDirectoryDefinesToolchain_ReturnsGrandparentToolchain()
    {
        using var workspace = TemporaryWorkspace.Create(outputHelper);
        File.WriteAllText(Path.Combine(workspace.WorkspaceRoot.FullName, "package.json"), "{ \"packageManager\": \"bun@1.2.0\" }");
    ...
    [Fact]
    public void Resolve_WhenWorkspaceRootLockFileIsSeveralLevelsAbove_ReturnsWorkspaceToolchain()
    {
        using var workspace = TemporaryWorkspace.Create(outputHelper);

        // Bun monorepo: the workspace lockfile lives at the repository root while the
        // AppHost is nested inside a brownfield package (apps/web/aspire-apphost).
        ...
        var webDirectory = workspace.WorkspaceRoot.CreateSubdirectory("apps").CreateSubdirectory("web");
        File.WriteAllText(Path.Combine(webDirectory.FullName, "package.json"), "{ \"name\": \"web\" }");
        var appHostDirectory = webDirectory.CreateSubdirectory("aspire-apphost");
        File.WriteAllText(Path.Combine(appHostDirectory.FullName, "package.json"), "{ \"name\": \"apphost\" }");

        var resolution = TypeScriptAppHostToolchainResolver.ResolveWithReason(appHostDirectory);

        Assert.Equal(TypeScriptAppHostToolchain.Bun, resolution.Toolchain);
        Assert.Equal($"bun.lock found in {workspace.WorkspaceRoot.FullName}", resolution.Reason);
    }

    [Fact]
    public void Resolve_WhenAncestorYarnLockIsClassic_Throws()
    {
        using var workspace = TemporaryWorkspace.Create(outputHelper);

        // A legacy classic yarn.lock several levels above the AppHost still governs resolution,
        // so the unsupported-toolchain error is surfaced rather than silently falling back to npm.
        var yarnLockPath = Path.Combine(workspace.WorkspaceRoot.FullName, "yarn.lock");
        File.WriteAllText(yarnLockPath, "# THIS IS AN AUTOGENERATED FILE. DO NOT EDIT THIS FILE DIRECTLY.\n# yarn lockfile v1\n");

        var appHostDirectory = workspace.WorkspaceRoot.CreateSubdirectory("apps").CreateSubdirectory("web").CreateSubdirectory("aspire-apphost");
        File.WriteAllText(Path.Combine(appHostDirectory.FullName, "package.json"), "{ \"name\": \"apphost\" }");

        var exception = Assert.Throws<YarnClassicNotSupportedException>(() => TypeScriptAppHostToolchainResolver.ResolveWithReason(appHostDirectory));

        Assert.Equal($"Yarn Classic is not supported for TypeScript AppHosts. Upgrade the Yarn lockfile at {yarnLockPath} to Yarn 4 or later, or use npm, pnpm, or Bun.", exception.Message);
    }

    [Fact]
    public void Resolve_WhenMarkerOnlyInHomeDirectory_ReturnsNpm()
    {
        using var workspace = TemporaryWorkspace.Create(outputHelper);

        // Treat the workspace root as the user's home directory: a marker that lives directly in
        // home must not govern a project nested below it, so the upward walk stops before reaching it.
        File.WriteAllText(Path.Combine(workspace.WorkspaceRoot.FullName, "package.json"), "{ \"packageManager\": \"bun@1.2.0\" }");
        File.WriteAllText(Path.Combine(workspace.WorkspaceRoot.FullName, "bun.lock"), "{}");

        var appHostDirectory = workspace.WorkspaceRoot.CreateSubdirectory("project").CreateSubdirectory("aspire-apphost");
        File.WriteAllText(Path.Combine(appHostDirectory.FullName, "package.json"), "{ \"name\": \"apphost\" }");

        var resolution = TypeScriptAppHostToolchainResolver.ResolveWithReason(appHostDirectory, workspace.WorkspaceRoot.FullName);
