...
public class SdkDumpCommandTests(ITestOutputHelper outputHelper)
{
    ...
    /// <summary>
    /// <c>sdk dump</c> reports the versions it was <em>asked</em> to scan, not the versions NuGet
    /// resolved. That is deliberate and different from <c>sdk export</c>, which publishes documents
    /// keyed on the version and therefore pins the restore.
    /// </summary>
    /// <remarks>
    /// Nothing consumes <c>packages</c> as a resolved identity: <c>--format ci</c> — the format the
    /// checked-in <c>*.ats.txt</c> baselines use — omits the block entirely, and
    /// <c>generate-ats-diffs.yml</c> passes <c>.csproj</c> paths, which carry no version at all.
    /// </remarks>
    [Fact]
    public void SdkDumpRecordsTheRequestedPackageVersionRatherThanTheResolvedOne()
    {
        ...
        // The restore is a NuGet minimum, so 13.4.1 can satisfy it while `packages` still says
        // 13.4.0. Callers that need the two to agree use `sdk export`.
        Assert.False(reference!.RequireExactVersion);
        Assert.Equal("13.4.0", reference.GetRestoreVersionRange(forceExact: false));

        var capabilities = new CapabilitiesInfo
        {
            Packages = [new PackageInfo { Name = reference.Name, Version = reference.Version! }]
        };

        using var document = JsonDocument.Parse(InvokeFormatter("FormatJson", capabilities));
        var package = Assert.Single(document.RootElement.GetProperty("Packages").EnumerateArray());
        Assert.Equal("Aspire.Hosting.Redis", package.GetProperty("Name").GetString());
    ...
    /// </summary>
    [Fact]
    public void SdkDumpCiFormatCarriesNoPackageVersions()
    {
        var capabilities = new CapabilitiesInfo
        {
            Packages = [new PackageInfo { Name = "Aspire.Hosting.Redis", Version = "13.4.0" }]
        };

        var sections = InvokeFormatter("FormatCi", capabilities)
            .Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries)
