...
public class NewCommandTests(ITestOutputHelper outputHelper)
{
    ...
    public async Task NewCommandNonInteractive_WithExplicitAssets_ConfiguresOnlySelectedClientsAtOutputRoot()
    {
        ...
        Assert.Equal(["claude", "copilot"], request.Environments.Select(client => client.Id));
    }
    ...
    [Fact]
    public async Task NewCommandNonInteractive_NoDetectedClients_CreatesProjectWithoutAgentConfiguration()
    {
        using var workspace = TemporaryWorkspace.CreateForCli(outputHelper);
        var services = CreateServiceCollection(workspace, options =>
        {
        ...
        var services = CreateServiceCollection(workspace, options =>
        {
            ...
            copilot.GetTargetsCallback = request =>
                [copilot.CreateTarget(Path.Combine(request.WorkspaceRoot.FullName, "settings.json"),
                    AgentAssetKind.AspireSkills, AgentConfigurationStatus.Blocked, "Existing source is pinned.")];
        });
        using var provider = services.BuildServiceProvider();
