...
public static class FakeArchiveHelper
{
    public static async Task<FakeArchive> CreateFakeArchiveAsync(string outputDir, string platform = "linux-x64")
    {
        ...
        try
        {
            ...
            {
                await CreateTarGzAsync(contentDir, archivePath, binaryName);
            }
    ...
    private static async Task CreateTarGzAsync(string contentDir, string archivePath, string binaryName)
    {
        using var tar = new Process
        {
            ...
            {
                FileName = "tar",
                Arguments = $"czf \"{archivePath}\" -C \"{contentDir}\" {binaryName}",
                RedirectStandardOutput = true,
                RedirectStandardError = true,
                UseShellExecute = false
            }
        ...
        if (tar.ExitCode != 0)
        {
            var stderr = await tar.StandardError.ReadToEndAsync();
            throw new InvalidOperationException($"tar failed with exit code {tar.ExitCode}: {stderr}");
        }
    }
