...
{
    Task<AppHostProjectInfo> GetAppHostInfoAsync(FileInfo projectFile, CancellationToken cancellationToken, bool noEvaluate = false);
}
...
internal sealed class AppHostInfoResolver(IDotNetCliRunner runner, IAppHostInfoDiskCache diskCache) : IAppHostInfoResolver
{
    ...
    public async Task<AppHostProjectInfo> GetAppHostInfoAsync(FileInfo projectFile, CancellationToken cancellationToken, bool noEvaluate = false)
    {
    ...
    private Task<AppHostProjectInfo> GetOrAddSharedFetch((string Path, DateTime LastWriteUtc, bool NoEvaluate) key, FileInfo projectFile)
    {
        while (true)
        {
    ...
    private async Task CompleteSharedFetchAsync((string Path, DateTime LastWriteUtc, bool NoEvaluate) key, FileInfo projectFile, TaskCompletionSource<AppHostProjectInfo> completion)
    {
        try
        {
            var result = await FetchAppHostInfoCoreAsync(projectFile, CancellationToken.None, noEvaluate: key.NoEvaluate).ConfigureAwait(false);
    ...
    private async Task<AppHostProjectInfo> FetchAppHostInfoCoreAsync(FileInfo projectFile, CancellationToken cancellationToken, bool noEvaluate = false)
    {
