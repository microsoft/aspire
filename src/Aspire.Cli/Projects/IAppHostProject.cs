...
internal interface IAppHostProject
{
    ...
    /// <param name="noEvaluate">Avoid expensive project evaluation.</param>
    /// <param name="cancellationToken">A cancellation token.</param>
    /// <returns>A validation result indicating if the file is valid and any additional status.</returns>
    Task<AppHostValidationResult> ValidateAppHostAsync(FileInfo appHostFile, bool noEvaluate, CancellationToken cancellationToken);
