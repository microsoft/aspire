...
/// or the private SDK path configured by <see cref="IDotNetRuntimeSelector"/>.
/// </summary>
internal sealed class DotNetSdkInstaller(IFeatures features, IConfiguration configuration, IDotNetRuntimeSelector runtimeSelector) : IDotNetSdkInstaller
{
    /// <summary>
    ...
    public async Task<bool> CheckAsync(string minimumVersion, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // Use the dotnet executable from the runtime selector (may be a private SDK path)
            // This ensures we check the correct SDK after private SDK installation.
            var dotnetExecutable = runtimeSelector.DotNetExecutablePath;
