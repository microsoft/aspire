...
internal sealed class MacOSCertificateManager : CertificateManager
{
    ...
    private static readonly string s_macOSUserKeychain = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile) + "/Library/Keychains/login.keychain-db";
    ...
    private static readonly string s_macOSTrustCertificateCommandLineArguments = $"add-trusted-cert -p basic -p ssl -k \"{s_macOSUserKeychain}\" ";
    ...
    private static readonly string s_macOSAddCertificateToKeyChainCommandLineArgumentsFormat = "import \"{0}\" -k \"" + s_macOSUserKeychain + "\" -t cert -f pkcs12 -P {1} -A";
...
{output}");
    ...
    {
        var maxRegexTimeout = TimeSpan.FromMinutes(1);
    ...
    private void SaveCertificateToUserKeychain(X509Certificate2 certificate)
    {
        ...
        var processInfo = new ProcessStartInfo(
            ...
            string.Format(CultureInfo.InvariantCulture, s_macOSAddCertificateToKeyChainCommandLineArgumentsFormat, certificatePath, password))
        {
            RedirectStandardOutput = true,
            RedirectStandardError = true
        };
    ...
    {
        if (store.Name! == StoreName.My.ToString() && store.Location == StoreLocation.CurrentUser && Directory.Exists(s_macOSUserHttpsCertificateLocation))
        {
...
#pragma warning restore CS8769 // Nullability of reference types in type of parameter doesn't match implemented member (possibly because of nullability attributes).
    ...
    private ICollection<X509Certificate2> GetCertsFromDisk()
    {
        ...
        if (!Directory.Exists(s_macOSUserHttpsCertificateLocation))
        {
            Log.MacOSDiskStoreDoesNotExist();
        }
        else
        {
            var certificateFiles = Directory.EnumerateFiles(s_macOSUserHttpsCertificateLocation, "aspnetcore-localhost-*.pfx");
            foreach (var file in certificateFiles)
            {
                try
                {
                    var certificate = X509CertificateLoader.LoadPkcs12FromFile(file, password: null);
    ...
    protected override void RemoveCertificateFromUserStoreCore(X509Certificate2 certificate)
    {
        ...
        if (IsCertOnKeychain(s_macOSUserKeychain, certificate))
        {
            RemoveCertificateFromKeychain(s_macOSUserKeychain, certificate);
        }
    }
