...
internal sealed partial class DocsIndexService(IDocsFetcher docsFetcher, IDocsCache docsCache, IConfiguration configuration, ILogger<DocsIndexService> logger) : IDocsIndexService
{
    ...
    private static string NormalizeCodeBlock(string codeBlock)
    {
        ...
        if (!content.Contains('\n'))
        {
            ...
            if (firstWhitespace > 0)
            {
                ...
                {
                    return $"\n```{language}\n{FormatMinifiedCodeBlock(language, code)}\n```\n";
                }
            }
    ...
    {
        if (code.Contains('\n') || !ShouldFormatMinifiedCode(language))
        {
            return code;
        }

        var builder = new StringBuilder(code.Length + 32);
        ...
        for (var index = 0; index < code.Length; index++)
        {
            ...
            if (inLineComment)
            {
                if (character == ' ' && LooksLikeCodeStart(code, index + 1))
                {
            ...
            if (character == ' ' && ShouldBreakBeforeChain(code, index + 1))
            {
                if (builder.Length > 0 && builder[^1] != '\n')
                {
                    builder.Append('\n');
        ...
        return TrailingWhitespaceBeforeNewlineRegex().Replace(builder.ToString().Trim(), "\n");
    }

    private static bool ShouldFormatMinifiedCode(string language)
        => language.ToLowerInvariant() is
            "csharp" or "cs" or
            "javascript" or "js" or
    ...
    {
        for (var current = index; current < code.Length; current++)
        {
    ...
    private static bool LooksLikeCodeStart(string code, int index)
    {
        ...
        if (token is "var" or "const" or "let" or "await" or "return" or "builder" or
            "if" or "for" or "while" or "using" or "public" or "private" or "protected" or
            "internal" or "class" or "interface" or "new")
        {
            return true;
        }

        return end < code.Length && code[end] is '(' or '.' or '=';
    }

    private static bool ShouldBreakBeforeChain(string code, int index)
    {
        index = SkipWhitespace(code, index);
        ...
        index++;
        if (index >= code.Length || !IsIdentifierStart(code[index]))
        {
            return false;
        }
        ...
        return code[index..end] is
            "With" or "Without" or "Add" or "Build" or "Run" or "Use" or "Create" or "Publish" or "Wait" or "Attach" or
            "with" or "without" or "add" or "build" or "run" or "use" or "create" or "publish" or "wait" or "attach";
    }
