...
internal static class CachedHttpDocumentFetcher
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var cachedETag = await cache.GetETagAsync(url, cancellationToken).ConfigureAwait(false);
            ...
            if (response is { StatusCode: HttpStatusCode.NotModified })
            {
                ...
                var cached = await cache.GetAsync(url, cancellationToken).ConfigureAwait(false);
                ...
                await cache.SetETagAsync(url, null, cancellationToken).ConfigureAwait(false);
                ...
                {
                    await cache.SetETagAsync(url, retryETag, cancellationToken).ConfigureAwait(false);
                }

                await cache.SetAsync(url, retryContent, cancellationToken).ConfigureAwait(false);
            ...
            {
                await cache.SetETagAsync(url, newETag, cancellationToken).ConfigureAwait(false);
                logger.LogDebug("Stored new ETag for {Url}: {ETag}", url, newETag);
            }

            await cache.SetAsync(url, content, cancellationToken).ConfigureAwait(false);
