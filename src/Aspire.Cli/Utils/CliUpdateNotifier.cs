...
internal class CliUpdateNotifier(
    ...
    IInstallationDetector installationDetector) : ICliUpdateNotifier
{
    ...
    public void NotifyIfUpdateAvailable()
    {
        ...
        {
            var installInfo = installationDetector.GetInstallationInfo();

            if (installInfo.SelfUpdateDisabled)
            {
                // Suppress the update notification entirely when self-update is disabled
                // (e.g., installed via WinGet/Homebrew)
                return;
            }

            var updateCommand = installInfo.UpdateInstructions ?? "aspire update --self";
