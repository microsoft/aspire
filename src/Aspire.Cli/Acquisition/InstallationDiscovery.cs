...
internal sealed partial class InstallationDiscovery : IInstallationDiscovery
{
            ...
            switch (probe)
            {
                case PeerProbeResult.Ok ok:
                    ...
                    if (string.IsNullOrEmpty(channel))
                    {
                        ...
                        // `--info --self` self-describe contract — they
    ...
    /// <c>--info --self</c> self-describe contract: those fall through to
    ...
    private string? TryReadChannel()
    {
        ...
        {
            // Same defensive posture as discovery: a misconfigured dev build
