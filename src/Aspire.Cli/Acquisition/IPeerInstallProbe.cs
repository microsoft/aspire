...
/// can't survive past <c>aspire info --all</c>'s lifetime.
/// </summary>
internal interface IPeerInstallProbe
{
    /// <summary>
    /// Runs <c><paramref name="binaryPath"/> info --format json</c> and
    /// returns either the parsed <see cref="InstallationInfo"/> or a failure
    /// reason. Never throws for ordinary peer-probe failures (timeout,
    /// non-zero exit, invalid JSON, missing executable); reserve exceptions
