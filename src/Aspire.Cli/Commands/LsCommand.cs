...
internal sealed class LsCommand : BaseCommand
{
    ...
    protected override async Task<CommandResult> ExecuteAsync(ParseResult parseResult, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            using (var findAppHostsActivity = _profilingTelemetry.StartLsFindAppHosts(scope.ToString()))
            {
                ...
                {
                    (true, _) => await FindAppHostsWithStatusAsync(scope, cancellationToken, noEvaluate).ConfigureAwait(false),
                    (_, true) => await FindAppHostsWithJsonStreamAsync(scope, cancellationToken, noEvaluate).ConfigureAwait(false),
                    _ => await _projectLocator.FindAppHostProjectsAsync(_executionContext.WorkingDirectory, scope, cancellationToken, noEvaluate).ConfigureAwait(false)
                };
    ...
    private async Task<List<AppHostProjectCandidate>> FindAppHostsWithJsonStreamAsync(AppHostDiscoveryScope scope, CancellationToken cancellationToken, bool noEvaluate = false)
    {
        ...
        await foreach (var candidate in _projectLocator.FindAppHostProjectsStreamAsync(_executionContext.WorkingDirectory, scope, cancellationToken: cancellationToken, noEvaluate: noEvaluate).ConfigureAwait(false))
        {
    ...
    private async Task<List<AppHostProjectCandidate>> FindAppHostsWithStatusAsync(AppHostDiscoveryScope scope, CancellationToken cancellationToken, bool noEvaluate = false)
    {
        var appHosts = new List<AppHostProjectCandidate>();
