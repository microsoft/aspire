        ...
        try
        {
            ...
            if (configOnlyGuestProject is not null && configOnlyProjectDirectory is not null)
            {
                ...
                var success = await _interactionService.ShowStatusAsync(
                    ...
                    async () => await configOnlyGuestProject.BuildAndGenerateSdkAsync(configOnlyProjectDirectory, updateDependencies: false, cancellationToken: cancellationToken),
            ...
            if (project is GuestAppHostProject guestProject)
            {
                ...
                var success = await _interactionService.ShowStatusAsync(
                    RestoreCommandStrings.RestoringSdkCode,
                    async () => await guestProject.BuildAndGenerateSdkAsync(directory, updateDependencies: false, cancellationToken: cancellationToken),
                    emoji: KnownEmojis.Gear);
