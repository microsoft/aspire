    ...
    ILogger<PlaywrightCliInstaller> logger)
{
    ...
    private static HashSet<string> SnapshotPlaywrightSkillDirs(string repoRoot)
    {
        ...
        foreach (var location in AgentAssetLocation.All)
        {
            var dir = Path.Combine(repoRoot, location.RelativeDirectory, PlaywrightCliSkillName);
            if (Directory.Exists(dir))
            {
                existing.Add(location.RelativeDirectory);
            }
        }
        return existing;
    }
    ...
    private void MirrorSkillFiles(string repoRoot, IReadOnlySet<string> selectedSkillDirectories, HashSet<string> preExistingLocations)
    {
        ...
        foreach (var location in AgentAssetLocation.All)
        {
            if (selectedSkillDirectories.Contains(location.RelativeDirectory))
            {
                continue; // User selected this location — keep it
            }

            if (preExistingLocations.Contains(location.RelativeDirectory))
            {
                continue; // Was already there before this run — leave it alone
            }

            var skillDir = Path.Combine(repoRoot, location.RelativeDirectory, PlaywrightCliSkillName);
            ...
            try
            {
                ...
                RemoveEmptyParentDirectories(skillDir, repoRoot, location.RelativeDirectory, logger);
            }
            catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
            {
