...
public partial class InteractionsProvider : ComponentBase, IAsyncDisposable
{
    ...
    {
        var message = GetMessageHtml(item);
        var inputs = item.InputsDialog is { } inputsDialog
            ? inputsDialog.InputItems.Select(MapInput).ToList()
            : new List<DeckInteractionInput>();

        // Publish the dialog only after its derived state is ready so observers never see a
        // partially initialized interaction.
        _currentDialogMessage = message;
        _currentDialogInputs = inputs;
        _currentDialog = item;
    }
