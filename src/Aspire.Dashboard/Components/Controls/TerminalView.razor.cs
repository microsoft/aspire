...
public sealed partial class TerminalView : ComponentBase, IAsyncDisposable
{
    ...
    public async Task ReconnectAsync(string? newResourceName, int newReplicaIndex)
    {
        ...
        try
        {
            ...
            ResourceName = newResourceName;
            ReplicaIndex = newReplicaIndex;
            await _jsModule.InvokeAsync<int>("reconnectTerminal", _terminalId, BuildWebSocketUrl(newResourceName, newReplicaIndex));
        }
        catch (JSDisconnectedException)
        {
    ...
    /// <summary>
    /// Recalculates the terminal layout after its previously hidden host becomes visible.
    /// </summary>
