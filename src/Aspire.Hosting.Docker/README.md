...
> [!NOTE]
> Docker Compose creates a new named volume owned by `root`, so a project or executable published as a non-root image cannot write to the mount without extra configuration. Aspire does not yet generate that configuration, so today this requires running the service as root or pre-creating the target directory with the right ownership in your image. Tracked by [#19422](https://github.com/microsoft/aspire/issues/19422).
