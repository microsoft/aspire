...
Referencing a group does not also deploy the referencing application into it. To use the same group as both a data-plane dependency and a compute environment, explicitly select it with `WithComputeEnvironment(sandboxGroup)`. If it is the only compute environment, `PublishAsAzureSandbox()` also opts a workload into sandbox deployment while configuring its runtime options. Compute selection is independent of the order of these calls and `WithReference`. Data-plane-only groups do not provision a default registry or image-pull identity.
