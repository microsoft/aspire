...
/// <summary>
/// Represents an Azure Connector Namespace trigger configuration.
/// </summary>
[AspireExport]
[Experimental("ASPIREAZURE001", UrlFormat = "https://aka.ms/aspire/diagnostics/{0}")]
public sealed class AzureConnectorGatewayTriggerConfigResource : AzureProvisioningResource, IResourceWithParent<AzureConnectorGatewayResource>, IResourceWithoutLifetime
{
        ...
        : base(name, ConfigureTriggerInfrastructure)
    {
        ...
        TriggerParameters = triggerParameters.ToArray();
    ...
    private static string? NormalizeCallbackPath(string? callbackPath)
    {
        ...
        var normalizedPath = callbackPath.Trim().TrimStart('/');
        if (normalizedPath.Contains('?') || normalizedPath.Contains('#'))
        {
            throw new ArgumentException(
                "The callback path cannot contain a query string or fragment.",
                nameof(callbackPath));
        }

        // ADC port URLs are stored as absolute root URIs with a trailing slash, for example:
        //   https://<sandbox-id>--8080.<region>.adcproxy.io/
        // Keep the callback path relative so deferred concatenation produces exactly one slash.
    ...
    private static void ConfigureTriggerInfrastructure(AzureResourceInfrastructure infrastructure)
    {
        ...
        var callbackUrlParameterName = global::Azure.Provisioning.Infrastructure.NormalizeBicepIdentifier(
            $"{triggerResource.Name}_callbackUrl");
        trigger.NotificationDetails.CallbackUrl = triggerResource.GetCallbackUrlExpression().AsProvisioningParameter(infrastructure, callbackUrlParameterName);
