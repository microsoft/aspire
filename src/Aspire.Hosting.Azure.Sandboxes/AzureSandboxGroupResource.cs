    ...
    IResourceWithCustomWithReference<AzureSandboxGroupResource>
{
    ...
    internal HashSet<IResource> ExplicitComputeResources { get; } = [];

    internal bool RequiresAcrPullIdentity => ExplicitComputeResources.Count > 0;

    IAzureContainerRegistryResource? IAzureComputeEnvironmentResource.ContainerRegistry =>
        HasDataPlaneReferences && !RequiresAcrPullIdentity ? null : ContainerRegistry;
    ...
    private async Task PrepareDeploymentTargetsAsync(PipelineStepContext context)
    {
        ...
        // Callers can still explicitly select this group with WithComputeEnvironment.
        ...
        var computeResources = context.Model.GetComputeResources()
            .Where(resource =>
            {
                if (HasDataPlaneReferences && !ExplicitComputeResources.Contains(resource))
                {
                    return false;
                }
