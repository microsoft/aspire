...
public static class AzureKubernetesEnvironmentExtensions
{
    ...
    private static Task ConfigureAksDefaultIngress(KubernetesIngressContext ctx)
    {
        ...
        {
            // Use the endpoint name as the service backend port name.
            // The K8s Service generated by ToService() uses the endpoint name
            // as the port name, so referencing by name always matches correctly
            // regardless of the actual port number.
            var backendPortName = endpoint.Name;
