...
public sealed class KubernetesEnvironmentResource : Resource, IComputeEnvironmentResource
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            claim.Metadata.Annotations[key] = await ResolveExpressionAsync(value, cancellationToken).ConfigureAwait(false);
        }
        ...
        var capacity = volumeResource.Capacity is { } capacityExpression
            ? await ResolveExpressionAsync(capacityExpression, cancellationToken).ConfigureAwait(false)
            : DefaultStorageSize;
        claim.Spec.Resources.Requests.Add("storage", capacity);

        if (volumeResource.StorageClassName is { } storageClassExpression)
        {
            var storageClass = await ResolveExpressionAsync(storageClassExpression, cancellationToken).ConfigureAwait(false);
