...
// https://pnpm.io/pnpm-workspace\_yaml#packages
...
internal static class PnpmWorkspaceYamlParser
{
    ...
    .Build();
    ///
    /// Reads workspace patterns from the packages: field of a
    /// pnpm-workspace.yaml document.
    ///
    /// The textual content of a pnpm-workspace.yaml file.
    ///
    /// The list of pattern strings declared under packages, in declaration order.
    /// Returns an empty list when the document is invalid YAML, the root is not a mapping,
    /// the packages key is missing, or its value is not a sequence.
    ///
    ...
    ///
    /// Reads the optional injectWorkspacePackages setting used by pnpm 10's non-legacy
    /// pnpm deploy implementation.
    ///
    ///
    ///  or  when the setting is present as a
    /// boolean scalar; otherwise .
    ///
