...
public static class BrowserLogsBuilderExtensions
{
    ...
    /// and <c>Aspire:Hosting:BrowserLogs:UserDataMode</c>, or scoped to a specific resource with
    /// <c>Aspire:Hosting:BrowserLogs:{ResourceName}:Browser</c>,
    /// <c>Aspire:Hosting:BrowserLogs:{ResourceName}:Profile</c>, and
    /// <c>Aspire:Hosting:BrowserLogs:{ResourceName}:UserDataMode</c>. Explicit method arguments override configuration.
    /// </para>
    /// </remarks>
    /// <example>
    ...
    /// <ats-param name="profile">Optional Chromium profile name or directory name to use. Only valid when the effective user data mode is <ats-see cref="!:type:BrowserUserDataMode.Shared" />. When not specified, the tracked browser uses the configured value from <c>Aspire:Hosting:BrowserLogs</c> if present.</ats-param>
    /// <ats-param name="userDataMode">Optional <ats-see cref="!:type:BrowserUserDataMode" /> that selects whether the tracked browser launches against a persistent Aspire-managed user data directory shared across all AppHosts on the machine (<ats-see cref="!:type:BrowserUserDataMode.Shared" />, the default) or a per-AppHost persistent user data directory (<ats-see cref="!:type:BrowserUserDataMode.Isolated" />). Both modes use Aspire-managed paths under <c>%LocalAppData%\Aspire\BrowserData</c> on Windows (or platform equivalents); the user's normal browser profile is never used. When not specified, the tracked browser uses the configured value from <c>Aspire:Hosting:BrowserLogs</c> and otherwise defaults to <ats-see cref="!:type:BrowserUserDataMode.Shared" />.</ats-param>
    [Experimental("ASPIREBROWSERLOGS001", UrlFormat = "https://aka.ms/aspire/diagnostics/{0}")]
    [AspireExport]
