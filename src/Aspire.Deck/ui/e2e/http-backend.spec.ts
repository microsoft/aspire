...
import { expectSelectedValue, selectOption } from "./select-helpers";
...
test.afterEach(async ({ page }) => {
  ...
  if (allowViteLoginFetchFailure.has(page)) {
    ...
    // document navigation. Depending on whether Vite handles that request before
    // navigation wins, it can emit one 500 JSON fallback; never allow more than one.
    expect(expectedViteLoginFailures.length, "At most one Vite login fetch fallback is expected").toBeLessThanOrEqual(1);
  }
...
test(`${features("AOT-SHELL-001")} keeps shell metadata, culture, and sign-out on versioned capabilities`, async ({ page }) => {
  ...
  await selectOption(page.getByRole("dialog", { name: "Settings" }).getByLabel("Language"), "fr");
  ...
  await expectSelectedValue(reloadedSettings.getByLabel("Language"), "fr");
...
test(`${features("AOT-CONTRACT-001", "HTTP-METRICS-001", "HTTP-METRIC-CLEAR-001")} owns metric polling, exact series queries, and clear through versioned AOT routes`, async ({ page }) => {
  ...
  await expectSelectedValue(metrics.getByRole("combobox", { name: "Resource" }), "stress-api");
  ...
  await expectSelectedValue(metrics.getByRole("combobox", { name: "Resource" }), "stress-worker");
  await metrics.getByRole("button", { name: "Clear metrics" }).click();
  await page.getByRole("menuitem", { name: "Clear all resources" }).click();
...
test(`${features("AOT-CONTRACT-001")} streams and deduplicates AOT resource console backlog and live output`, async ({ page }) => {
  ...
  await selectOption(page.getByRole("main").getByRole("combobox", { name: "Resource" }), resource.name);
...
test(`${features("AOT-CONTRACT-001", "HTTP-STRUCTURED-LOG-CLEAR-001")} owns structured-log replay, live updates, and clear through versioned AOT routes`, async ({ page }) => {
  ...
  await selectOption(region.getByRole("combobox", { name: "Resource" }), "stress-api");
...
test(`${features("HTTP-LANGUAGE-001")} selects and applies a dashboard language`, async ({ page }) => {
  ...
  await expectSelectedValue(language, "en");
  ...
  await expectSelectedValue(settings.getByLabel("Language"), "fr");
...
test(`${features("HTTP-STRUCTURED-LOG-CLEAR-001")} clears selected and all structured logs through the HTTP backend`, async ({ page }, testInfo) => {
  ...
  await selectOption(logs.getByRole("combobox", { name: "Resource" }), "stress-api");
...
test(`${features("HTTP-TRACE-CLEAR-001")} clears selected and all traces through the HTTP backend`, async ({ page }) => {
  ...
  await selectOption(traces.getByRole("combobox", { name: "Resource" }), "stress-api");
  await traces.getByRole("button", { name: "Clear traces" }).click();
  await page.getByRole("menuitem", { name: "Clear stress-api" }).click();
