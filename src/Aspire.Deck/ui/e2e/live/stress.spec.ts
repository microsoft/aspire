...
test.afterEach(async ({ page }) => {
  ...
  if (allowNavigationAbort.has(page)) {
    unexpected = unexpected.filter((error) =>
      !/^request: GET .*\/(?:api\/deck\/(?:telemetry\/(?:(?:logs|spans)\?follow=true|metrics(?:\/series\?.*)?)|interactions|resources(?:\/[^/]+\/console-logs)?)|api\/dashboard\/v1\/(?:traces\?.*|metrics\/series\?.*|interactions)) \(net::ERR_ABORTED\)$/.test(error));
  }
