...
import { expectSelectedValue, expectSelectOptions, selectOption } from "./select-helpers";
...
test(`${features("CONSOLE-RESOURCE-001", "CONSOLE-ALL-001", "CONSOLE-STREAM-001", "CONSOLE-SWITCH-001", "CONSOLE-FOLLOW-001")} streams and follows all or one resource console`, async ({ page }) => {
  ...
  await expectSelectedValue(resourceSelect, "__all-resources__");
  ...
  await selectOption(resourceSelect, "frontend");
  await expectSelectedValue(resourceSelect, "frontend");
  ...
  await selectOption(resourceSelect, "cache");
  await expectSelectedValue(resourceSelect, "cache");
...
test(`${features("CONSOLE-TERMINAL-001", "CONSOLE-TERMINAL-FONT-001", "CONSOLE-TERMINAL-SIZE-001")} controls an interactive terminal session`, async ({ page }) => {
  await navigationButton(page, "Console").click();
  await selectOption(page.getByRole("combobox", { name: "Resource" }), "migration");
  ...
  await selectOption(terminal.getByRole("combobox", { name: "Terminal grid size" }), "100x30");
...
test(`${features("CONSOLE-ROUTE-001")} restores console resource and display options from the URL`, async ({ page }) => {
  await navigationButton(page, "Console").click();
  await selectOption(page.getByRole("combobox", { name: "Resource" }), "cache");
  ...
  await page.reload();
  await expectSelectedValue(page.getByRole("combobox", { name: "Resource" }), "cache");
...
test(`${features("LOG-LIST-001", "LOG-FILTER-001", "LOG-SEVERITY-001", "LOG-LIVE-001")} lists, filters, and updates structured logs`, async ({ page }) => {
  ...
  await expectSelectOptions(severity, [
    "All",
    "Trace",
    "Debug",
    "Information",
  ...
  await selectOption(severity, "Warning");
  await expect.poll(() => dataRows.count()).toBeGreaterThan(0);
  await expect
...
test(`${features("LOG-GENAI-001", "TRACE-GENAI-001")} opens GenAI conversations from logs and spans`, async ({ page }) => {
  ...
  await page.getByRole("switch", { name: "Pause incoming data" }).check();
  await selectOption(page.getByRole("combobox", { name: "Span type" }), "genai");
...
test(`${features("LOG-RESOURCE-001", "LOG-PAUSE-001")} filters resources and pauses incoming structured logs`, async ({ page }) => {
  ...
  const resourceOptions = page.getByRole("option");
  await expect.poll(() => resourceOptions.count()).toBeGreaterThan(1);
  const resourceName = await resourceOptions.nth(1).locator("[data-option-value]").getAttribute("data-option-value");
  expect(resourceName).not.toBeNull();
  await page.keyboard.press("Escape");
  await selectOption(resource, resourceName!);
  await expect.poll(() => rows.count()).toBeGreaterThan(0);
  await expect.poll(async () =>
    (await rows.locator("td:nth-child(1)").allTextContents()).every((value) => value.trim() === resourceName),
  ).toBe(true);
  await selectOption(resource, "all");
...
test(`${features("LOG-STRUCTURED-FILTER-001", "LOG-FILTER-COUNT-001", "LOG-ROUTE-001")} manages and restores structured log filters`, async ({ page }) => {
  ...
  await selectOption(logs.getByRole("combobox", { name: "Resource" }), "frontend");
  await selectOption(logs.getByRole("combobox", { name: "Severity" }), "Error");

  await logs.getByRole("button", { name: "Add filter" }).click();
  const dialog = page.getByRole("dialog", { name: "Add filter" });
  await selectOption(dialog.getByRole("combobox", { name: "Field" }), "http.request.method");
  await selectOption(dialog.getByRole("combobox", { name: "Condition" }), "equals");
  ...
  await page.reload();
  await expectSelectedValue(logs.getByRole("combobox", { name: "Resource" }), "frontend");
  await expectSelectedValue(logs.getByRole("combobox", { name: "Severity" }), "Error");
...
test(`${features("TRACE-STRUCTURED-FILTER-001")} composes and restores structured trace filters`, async ({ page }) => {
  ...
  const dialog = page.getByRole("dialog", { name: "Add filter" });
  await selectOption(dialog.getByRole("combobox", { name: "Field" }), "db.system.name");
  await selectOption(dialog.getByRole("combobox", { name: "Condition" }), "equals");
...
test(`${features("TRACE-RESOURCE-001", "TRACE-TYPE-001", "TRACE-PAUSE-001", "TRACE-SESSION-001")} filters, pauses, and restores trace inventory state`, async ({ page }) => {
  ...
  await resource.click();
  await expect(page.locator('[data-slot="select-group"]')).toHaveCount(4);
  await page.keyboard.press("Escape");
  await selectOption(resource, "frontend");
  await expect.poll(() => traces.count()).toBeGreaterThan(0);
  expect(await traces.count()).toBeLessThan(pausedTraceCount);

  const type = tracesPage.getByRole("combobox", { name: "Span type" });
  await expectSelectOptions(type, [
    "All span types",
    "HTTP",
    "Database",
    "Messaging",
  ...
  await selectOption(type, "genai");
  await expect(page.getByText("No traces match your filter.", { exact: true })).toBeVisible();
  await selectOption(type, "database");
  await expect.poll(() => traces.count()).toBeGreaterThan(0);

  await tracesPage.getByRole("textbox", { name: "Filter traces…" }).fill("redis");
  await selectOption(tracesPage.getByRole("combobox", { name: "Min duration" }), "5");
  ...
  await expectSelectedValue(page.getByRole("combobox", { name: "Resource" }), "frontend");
  await expectSelectedValue(page.getByRole("combobox", { name: "Span type" }), "database");
  await expect(page.getByRole("textbox", { name: "Filter traces…" })).toHaveValue("redis");
  await expectSelectedValue(page.getByRole("combobox", { name: "Min duration" }), "5");
  await expect(page.getByRole("switch", { name: "Pause incoming data" })).toBeChecked();
...
test(`${features("TRACE-CLEAR-001")} clears selected and all trace telemetry`, async ({ page }) => {
  ...
  await tracesPage.getByRole("switch", { name: "Pause incoming data" }).check();
  await selectOption(tracesPage.getByRole("combobox", { name: "Resource" }), "frontend");
  ...
  await expectSelectedValue(tracesPage.getByRole("combobox", { name: "Resource" }), "all");
...
test(`${features("METRIC-LIST-001", "METRIC-RESOURCE-001", "METRIC-SELECT-001", "METRIC-TREE-001", "METRIC-METADATA-001", "METRIC-CHART-001", "METRIC-HISTOGRAM-001", "METRIC-CURSOR-001", "METRIC-PAUSE-001", "METRIC-RANGE-001", "METRIC-TABLE-001", "METRIC-DIMENSIONS-001", "METRIC-EXEMPLARS-001", "METRIC-ROUTES-001", "METRIC-SESSION-001", "METRIC-ZOOM-001")} explores and restores live metric series`, async ({ page }, testInfo) => {
  ...
  await expectSelectOptions(resource, ["apiservice", "frontend", "cache", "postgres"]);
  await expectSelectedValue(resource, "apiservice");
  await selectOption(resource, "frontend");
...
test(`${features("METRIC-CLEAR-001")} clears selected and all metric telemetry`, async ({ page }) => {
  await navigationButton(page, "Metrics").click();
  const metricsPage = page.getByRole("main").getByRole("region", { name: "Metrics" });
  await selectOption(metricsPage.getByRole("combobox", { name: "Resource" }), "frontend");
  ...
  await expectSelectedValue(metricsPage.getByRole("combobox", { name: "Resource" }), "apiservice");
