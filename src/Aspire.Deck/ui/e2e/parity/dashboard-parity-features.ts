...
const featureDefinitions = {
  shell: [
    ...
    ["SHELL-NOTIFICATIONS-001", "/", "Active notifications render intent, actions, links, and dismiss behavior.", "shell", "partial", "APP-NOTIFICATION-001"],
    ["SHELL-NOTIFICATION-CENTER-001", "/", "The notification center opens and preserves notification history.", null, "missing"],
    ["SHELL-SETTINGS-001", "/", "Settings opens from the top bar and reports dashboard/runtime versions.", "shell", "missing"],
    ["SHELL-THEME-001", "/", "System, light, and dark theme selection is persisted.", "shell", "partial", "APP-THEME-001"],
    ["SHELL-LANGUAGE-001", "/", "The dashboard language can be selected and applied.", "shell", "missing"],
    ["SHELL-TIME-FORMAT-001", "/", "System, 12-hour, and 24-hour time formatting can be selected.", "shell", "missing"],
    ...
    ["SHELL-NOTFOUND-001", "/error/404", "Unknown routes render a dedicated not-found experience.", null, "missing"],
    ["SHELL-ERROR-001", "/error", "Unhandled errors render a recoverable error experience.", null, "missing"],
    ...
    ["SHELL-ACCESSIBILITY-001", "/", "Landmarks, names, focus order, dialogs, and keyboard interaction remain accessible.", "shell", "partial", "APP-PAGE-001; toolkit.aria.yml"],
  ...
  resources: [
    ...
    ["RES-HIDDEN-001", "/", "Hidden resources are excluded by default and can be shown through view options.", "resources", "partial", "STRESS-VISIBILITY-001"],
    ...
    ["RES-STRUCTURED-FILTER-001", "/", "Resource state, type, and health filters can be composed and cleared.", "resources", "missing"],
    ["RES-VIEW-OPTIONS-001", "/", "View options control hidden resources, resource type, and hierarchy expansion.", "resources", "missing"],
    ["RES-SORT-001", "/", "Supported resource columns sort ascending and descending.", "resources", "covered", "RES-SORT-001"],
    ["RES-COLUMNS-001", "/", "Name, state, source, URLs, and start time columns render the legacy data model.", "resources", "partial", "RES-LIST-001; RES-ENDPOINT-001"],
    ["RES-SOURCE-001", "/", "Project, executable, container, and custom resource sources are displayed.", "resources", "missing"],
    ["RES-URLS-001", "/", "External endpoints use display names and preserve internal/inactive endpoint rules.", "resources", "partial", "RES-ENDPOINT-001"],
    ...
    ["RES-NESTING-001", "/", "Parent/child resources render as a hierarchical tree.", "resources", "missing"],
    ["RES-EXPAND-001", "/", "Individual branches and all branches can be expanded and collapsed.", "resources", "missing"],
    ...
    ["RES-DETAILS-LINK-001", "/?resource={name}", "A resource details selection is deep-linkable and restorable.", null, "missing"],
    ["RES-PROPERTIES-001", "/", "Known, custom, highlighted, null, array, and object properties render correctly.", "resources", "partial", "RES-DETAILS-001"],
    ...
    ["RES-COPY-001", "/", "Resource property and environment values can be copied without accidental disclosure.", null, "missing"],
    ...
    ["RES-STATE-001", "/", "Running, starting, finished, exited, not-started, and unknown states remain distinguishable.", "resources", "partial", "STRESS-RESOURCES-001"],
    ["RES-NO-STATUS-001", "/", "Resources without status data render a stable unknown state.", "resources", "partial", "STRESS-RESOURCES-001"],
    ["RES-RESOURCE-ICON-001", "/", "Custom Fluent resource icon names override resource-type fallbacks.", null, "missing"],
    ["RES-RESOURCE-ICON-VARIANT-001", "/", "Regular and filled resource icon variants are preserved.", null, "missing"],
    ["RES-CONTEXT-MENU-001", "/", "Resource rows expose details, navigation, and commands through a context menu.", null, "missing"],
    ["RES-SESSION-001", "/", "Search, filters, sort, view, expansion, and selection survive navigation and reload.", null, "missing"],
  ],
  parameters: [
  ...
  commands: [
    ...
    ["CMD-HIGHLIGHT-001", "/", "Highlighted commands remain directly available and other commands use overflow presentation.", "commands", "partial", "RES-ACTION-MENU-001"],
    ["CMD-ICON-001", "/", "Custom command icon names render in direct and overflow command surfaces.", "commands", "missing"],
    ["CMD-ICON-VARIANT-001", "/", "Regular and filled command icon variants are preserved.", null, "missing"],
    ...
    ["CMD-CHOICE-001", "/", "Choice arguments preserve options, display names, placeholders, and defaults.", "commands", "partial", "RES-INTERACTION-001"],
    ["CMD-CUSTOM-CHOICE-001", "/", "Choice arguments can allow a searchable custom value.", null, "missing"],
    ["CMD-SECRET-001", "/", "Secret text arguments mask values, disable password saving, and support explicit reveal.", "commands", "missing"],
    ...
    ["CMD-VALIDATION-001", "/", "Field-level and form-level validation errors are announced and rendered.", null, "partial", "RES-INTERACTION-001"],
    ["CMD-MANY-INPUTS-001", "/", "Large command forms remain scrollable and submit every input value.", null, "missing"],
    ["CMD-MESSAGEBOX-001", "/", "Confirmation and message-box interactions support primary, secondary, dismiss, and intent.", null, "partial", "APP-NOTIFICATION-001"],
    ["CMD-NOTIFICATION-001", "/", "Interaction notifications support semantic intent, links, actions, and non-dismissible state.", null, "partial", "APP-NOTIFICATION-001"],
    ["CMD-MARKDOWN-001", "/", "Interaction messages and field descriptions opt into sanitized Markdown rendering.", null, "missing"],
    ["CMD-RESULT-TEXT-001", "/", "Plain-text command results open in a readable visualizer and can be downloaded.", null, "missing"],
    ["CMD-RESULT-JSON-001", "/", "JSON command results preserve formatting, masking, copy, and download behavior.", null, "missing"],
    ["CMD-RESULT-MARKDOWN-001", "/", "Markdown command results render tables and rich content safely.", null, "missing"],
    ["CMD-RESULT-IMMEDIATE-001", "/", "DisplayImmediately command results open without requiring a second action.", null, "missing"],
  ...
  console: [
    ...
    ["CONSOLE-PAUSE-001", "/consolelogs", "Incoming console data can be paused and resumed without losing context.", "console", "missing"],
    ["CONSOLE-CLEAR-001", "/consolelogs", "Console data can be cleared for the selected resource or all resources.", "console", "missing"],
    ["CONSOLE-DOWNLOAD-001", "/consolelogs", "The current console log can be downloaded.", null, "missing"],
    ["CONSOLE-TIMESTAMP-001", "/consolelogs", "Timestamp visibility and UTC/local formatting can be toggled.", null, "missing"],
    ["CONSOLE-WRAP-001", "/consolelogs", "Long console lines can wrap or scroll horizontally.", null, "missing"],
    ["CONSOLE-COMMANDS-001", "/consolelogs", "Commands for the selected resource are available from the console toolbar.", null, "missing"],
    ...
    ["CONSOLE-ROUTE-001", "/consolelogs/resource/{name}", "Selected resource and console options are deep-linkable and restorable.", null, "missing"],
  ],
  "structured-logs": [
    ...
    ["LOG-PAUSE-001", "/structuredlogs", "Incoming structured logs can be paused and resumed.", "structured-logs", "missing"],
    ...
    ["LOG-TRACE-LINK-001", "/structuredlogs", "Trace IDs deep-link to the matching trace and span.", null, "missing"],
  ...
  traces: [
    ...
    ["TRACE-STRUCTURED-FILTER-001", "/traces", "Structured trace and span filters can be composed and managed.", "traces", "missing"],
    ["TRACE-DURATION-001", "/traces", "Trace and span duration is represented consistently at different scales.", "traces", "partial", "TRACE-DURATION-001"],
    ["TRACE-ERROR-001", "/traces", "Failed traces and spans expose status, tags, and error styling.", null, "partial", "TRACE-ERROR-001"],
    ["TRACE-PAUSE-001", "/traces", "Incoming traces can be paused and resumed.", "traces", "missing"],
    ["TRACE-CLEAR-001", "/traces", "Trace data can be cleared for the selected resource or all resources.", "traces", "missing"],
    ["TRACE-VIRTUALIZATION-001", "/traces", "Large trace inventories remain responsive through virtualization.", null, "missing"],
    ["TRACE-ACTIONS-001", "/traces", "Per-trace actions expose detail, copy, and related telemetry navigation.", "traces", "missing"],
    ["TRACE-DETAIL-ROUTE-001", "/traces/detail/{traceId}", "A trace opens on a stable deep-linked detail route.", null, "missing"],
    ...
    ["TRACE-EXPAND-001", "/traces/detail/{traceId}", "Trace detail supports individual and expand-all/collapse-all span control.", null, "partial", "TRACE-COLLAPSE-001"],
    ["TRACE-SPAN-DETAILS-001", "/traces/detail/{traceId}", "Span details include identifiers, timing, status, attributes, resource, and instrumentation scope.", null, "partial", "TRACE-DETAILS-001"],
    ["TRACE-EVENTS-001", "/traces/detail/{traceId}", "Span events and exception details preserve timestamps and attributes.", null, "missing"],
    ["TRACE-LINKS-001", "/traces/detail/{traceId}", "Span links navigate to related traces and preserve link attributes.", null, "missing"],
    ["TRACE-GENAI-001", "/traces/detail/{traceId}", "GenAI spans and traces open the dedicated GenAI visualizer.", null, "missing"],
    ["TRACE-EXPLAIN-001", "/traces", "Explain errors summarizes current failed traces through the assistant.", null, "missing"],
    ["TRACE-SESSION-001", "/traces", "Resource, type, filters, and selection are deep-linkable and restorable.", null, "missing"],
  ],
  metrics: [
    ["METRIC-RESOURCE-001", "/metrics", "Metrics require and preserve a selected telemetry resource.", "metrics", "missing"],
    ["METRIC-TREE-001", "/metrics", "Meters and instruments render in a searchable hierarchical selector.", null, "missing"],
    ["METRIC-METADATA-001", "/metrics", "Instrument name, description, unit, type, and meter metadata are visible.", null, "partial", "METRIC-SELECT-001"],
    ["METRIC-DURATION-001", "/metrics", "One minute through twelve hour duration presets update the query window.", "metrics", "partial", "METRIC-RANGE-001"],
    ["METRIC-PAUSE-001", "/metrics", "Incoming metrics can be paused and resumed.", "metrics", "partial", "METRIC-PAUSE-001"],
    ["METRIC-CLEAR-001", "/metrics", "Metric data can be cleared for the selected resource or all resources.", "metrics", "missing"],
    ["METRIC-CHART-001", "/metrics", "Instrument samples render as an interactive time-series chart.", null, "partial", "METRIC-CHART-001"],
    ["METRIC-TABLE-001", "/metrics", "Metric data can switch between chart and table representations.", null, "missing"],
    ["METRIC-DIMENSIONS-001", "/metrics", "Dimension filters and multiple attribute series are discoverable and selectable.", null, "missing"],
    ["METRIC-HISTOGRAM-001", "/metrics", "Histogram count, sum, buckets, and percentile views preserve aggregation semantics.", null, "partial", "METRIC-CHART-001"],
    ["METRIC-EXEMPLARS-001", "/metrics", "Metric exemplars expose values, timestamps, attributes, and related trace navigation.", null, "missing"],
    ...
    ["METRIC-ROUTES-001", "/metrics/resource/{resource}/meter/{meter}/instrument/{instrument}", "Resource, meter, instrument, duration, and view are deep-linkable.", null, "missing"],
    ["METRIC-EMPTY-001", "/metrics", "Loading, no-resource, no-meter, no-instrument, and no-data states are distinct.", "metrics", "partial", "HTTP-EMPTY-TELEMETRY-001; STRESS-EMPTY-TELEMETRY-001"],
    ["METRIC-SESSION-001", "/metrics", "Metric selection, duration, view, dimensions, and zoom are restorable.", null, "missing"],
  ],
