...
const featureDefinitions = {
  shell: [
    ...
    ["SHELL-ASSISTANT-001", "/", "The AI assistant opens, closes, expands, starts a new chat, and sends or stops responses.", "assistant", "covered", "AOT-ASSISTANT-001; APP-ASSISTANT-001; HTTP-ASSISTANT-001; DeckApiTests.AssistantEndpoints_ReturnNotFoundWhenAssistantIsDisabled"],
    ...
    ["SHELL-UNSECURED-001", "/", "An unsecured telemetry/API endpoint warning is visible with supporting guidance.", "unsecured", "covered", "AOT-SHELL-001; HTTP-SHELL-UNSECURED-001; DeckApiTests.GetConfig_ReturnsDeckConfigContract"],
  ...
  commands: [
    ...
    ["CMD-ICON-001", "/", "Custom command icon names render in direct and overflow command surfaces.", "commands", "covered", "RES-ICON-001; STRESS-COMMAND-ICON-001"],
  ...
  console: [
    ...
    ["CONSOLE-BACKLOG-001", "/consolelogs", "Selecting a resource loads the existing console backlog.", "console", "covered", "CONSOLE-STREAM-001; HTTP-CONSOLE-001; AOT-CONTRACT-001; STRESS-CONSOLE-001"],
    ["CONSOLE-LIVE-001", "/consolelogs", "New stdout and stderr lines stream without reloading the page.", "console", "covered", "CONSOLE-STREAM-001; AOT-CONTRACT-001; STRESS-CONSOLE-001"],
    ["CONSOLE-SWITCH-001", "/consolelogs", "Switching resources replaces the visible stream and subscription.", "console", "covered", "CONSOLE-SWITCH-001; AOT-CONTRACT-001"],
    ...
    ["CONSOLE-PAUSE-001", "/consolelogs", "Incoming console data can be paused and resumed without losing context.", "console", "covered", "CONSOLE-PAUSE-001; CONSOLE-ROUTE-001; AOT-CONTRACT-001"],
    ...
    ["CONSOLE-TERMINAL-001", "/consolelogs", "Interactive resources render a terminal and can take control of input.", "terminal", "covered", "CONSOLE-TERMINAL-001; TERMINAL-LIVE-001; AOT-CONTRACT-001; DeckApiTests.GetResources_ReturnsTerminalMetadata"],
    ["CONSOLE-TERMINAL-FONT-001", "/consolelogs", "Interactive terminal font size can be increased and decreased.", "terminal", "covered", "CONSOLE-TERMINAL-FONT-001; TERMINAL-LIVE-001; AOT-CONTRACT-001"],
    ["CONSOLE-TERMINAL-SIZE-001", "/consolelogs", "Interactive terminal column and row presets update the remote terminal size.", "terminal", "covered", "CONSOLE-TERMINAL-SIZE-001; TERMINAL-LIVE-001; AOT-CONTRACT-001"],
  ...
  "structured-logs": [
    ["LOG-LIST-001", "/structuredlogs", "Structured logs render resource, level, timestamp, message, trace, and actions columns.", "structured-logs", "covered", "LOG-LIST-001; HTTP-STRUCTURED-LOGS-001; AOT-CONTRACT-001; STRESS-STRUCTURED-LOGS-001"],
    ["LOG-LIVE-001", "/structuredlogs", "New structured logs stream into the list and update totals.", "structured-logs", "covered", "LOG-LIVE-001; HTTP-STRUCTURED-LOGS-001; AOT-CONTRACT-001; STRESS-STRUCTURED-LOGS-001"],
    ["LOG-RESOURCE-001", "/structuredlogs", "Logs filter through a grouped resource selector.", "structured-logs", "covered", "LOG-RESOURCE-001; AOT-CONTRACT-001; STRESS-STRUCTURED-LOG-RESOURCE-001"],
    ["LOG-LEVEL-001", "/structuredlogs", "All supported severity levels can be selected.", "structured-logs", "covered", "LOG-SEVERITY-001"],
    ["LOG-TEXT-FILTER-001", "/structuredlogs", "Logs filter across resource and message content.", "structured-logs", "covered", "LOG-FILTER-001; AOT-CONTRACT-001"],
    ...
    ["LOG-PAUSE-001", "/structuredlogs", "Incoming structured logs can be paused and resumed.", "structured-logs", "covered", "LOG-PAUSE-001; AOT-CONTRACT-001; STRESS-STRUCTURED-LOG-PAUSE-001"],
    ["LOG-CLEAR-001", "/structuredlogs", "Structured logs can be cleared for the selected resource or all resources.", "structured-logs", "covered", "HTTP-STRUCTURED-LOG-CLEAR-001; AOT-CONTRACT-001; STRESS-STRUCTURED-LOG-CLEAR-001"],
    ["LOG-VIRTUALIZATION-001", "/structuredlogs", "Large log volumes remain responsive through row virtualization.", "structured-log-virtualization", "covered", "HTTP-STRUCTURED-LOG-VIRTUALIZATION-001"],
  ...
  traces: [
    ["TRACE-LIST-001", "/traces", "Traces render timestamp, name, span count, duration, error status, and actions.", "traces", "covered", "TRACE-LIST-001; TRACE-ACTIONS-001; HTTP-TRACES-001; AOT-CONTRACT-001; STRESS-TRACES-001"],
    ["TRACE-LIVE-001", "/traces", "Incoming spans update trace groups and totals without reloading the page.", "traces", "covered", "TRACE-LIVE-001; HTTP-TRACES-001; AOT-CONTRACT-001; STRESS-TRACES-001"],
    ...
    ["TRACE-CLEAR-001", "/traces", "Trace data can be cleared for the selected resource or all resources.", "traces", "covered", "TRACE-CLEAR-001; HTTP-TRACE-CLEAR-001; AOT-CONTRACT-001"],
  ...
  metrics: [
    ["METRIC-RESOURCE-001", "/metrics", "Metrics require and preserve a selected telemetry resource.", "metrics", "covered", "METRIC-RESOURCE-001; METRIC-SESSION-001; AOT-CONTRACT-001"],
    ["METRIC-TREE-001", "/metrics", "Meters and instruments render in a hierarchical selector.", "metrics", "covered", "METRIC-TREE-001"],
    ["METRIC-METADATA-001", "/metrics", "Instrument name, description, displayed unit, histogram mode, and containing meter are visible.", "metrics", "covered", "METRIC-METADATA-001"],
    ...
    ["METRIC-CLEAR-001", "/metrics", "Metric data can be cleared for the selected resource or all resources.", "metrics", "covered", "METRIC-CLEAR-001; AOT-CONTRACT-001; DeckApiTests.DeleteMetrics_ClearsSelectedOrAllResources"],
    ...
    ["METRIC-DIMENSIONS-001", "/metrics", "Dimension filters and multiple attribute series are discoverable and selectable.", "metrics", "covered", "METRIC-DIMENSIONS-001; AOT-CONTRACT-001; DeckApiTests.GetMetrics_ReturnsSummariesAndSeries"],
    ["METRIC-HISTOGRAM-001", "/metrics", "Histogram count and percentile views preserve aggregation semantics.", "metrics", "covered", "METRIC-HISTOGRAM-001; HTTP-METRICS-001; AOT-CONTRACT-001; DeckApiTests.GetMetrics_ReturnsSummariesAndSeries; ChartDataCalculatorTests.TryCalculateHistogramAggregatePoint_ReturnsCumulativeDeltas"],
    ["METRIC-EXEMPLARS-001", "/metrics", "Metric exemplars expose values, timestamps, attributes, and related trace navigation.", "metrics", "covered", "METRIC-EXEMPLARS-001; AOT-CONTRACT-001; DeckApiTests.GetMetrics_ReturnsSummariesAndSeries"],
