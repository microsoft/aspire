...
- React partial: 0
- React missing: 0
- Legacy black-box scenarios pending: 0
- React parity gaps: 0
...
| SHELL-ASSISTANT-001 | shell | `/` | Removed upstream | covered | 0a1a54a360; APP-SHELL-001; AOT-SHELL-001 | The retired AI assistant is not advertised or rendered after its removal from current main. |
...
| SHELL-MANAGE-DATA-001 | shell | `/` | manage-data | covered | AOT-MANAGE-DATA-001; HTTP-MANAGE-DATA-001; DeckApiTests.ManageData_InventoryExportImportAndRemoveUseDeckContract | Resource logs and telemetry can be inspected, exported, imported, and cleared. |
...
| RES-RESOURCE-ICON-001 | resources | `/` | resources | covered | RES-ICON-001; HTTP-RESOURCES-001; STRESS-RESOURCE-ICON-001 | Custom Aspire resource icon names override resource-type fallbacks. |
...
| CONSOLE-TERMINAL-001 | console | `/consolelogs` | terminal | covered | CONSOLE-TERMINAL-001; TERMINAL-LIVE-001; AOT-CONTRACT-001; DeckApiTests.GetResources_ReturnsTerminalMetadata | Interactive resources render a terminal and can take control of input. |
| CONSOLE-TERMINAL-FONT-001 | console | `/consolelogs` | terminal | covered | CONSOLE-TERMINAL-FONT-001; TERMINAL-LIVE-001; AOT-CONTRACT-001 | Interactive terminal font size can be increased and decreased. |
| CONSOLE-TERMINAL-SIZE-001 | console | `/consolelogs` | terminal | covered | CONSOLE-TERMINAL-SIZE-001; TERMINAL-LIVE-001; AOT-CONTRACT-001 | Interactive terminal column and row presets update the remote terminal size. |
...
| LOG-CLEAR-001 | structured-logs | `/structuredlogs` | structured-logs | covered | HTTP-STRUCTURED-LOG-CLEAR-001; AOT-CONTRACT-001; STRESS-STRUCTURED-LOG-CLEAR-001 | Structured logs can be cleared for the selected resource or all resources. |
...
| LOG-EXPLAIN-001 | structured-logs | `/structuredlogs` | Removed upstream | covered | 0a1a54a360; LOG-LIST-001 | The retired Explain errors assistant action remains absent after its removal from current main. |
...
| TRACE-EXPLAIN-001 | traces | `/traces` | Removed upstream | covered | 0a1a54a360; TRACE-LIST-001 | The retired Explain errors assistant action remains absent after its removal from current main. |
...
| METRIC-RESOURCE-001 | metrics | `/metrics` | metrics | covered | METRIC-RESOURCE-001; METRIC-SESSION-001; AOT-CONTRACT-001 | Metrics require and preserve a selected telemetry resource. |
...
| METRIC-CLEAR-001 | metrics | `/metrics` | metrics | covered | METRIC-CLEAR-001; AOT-CONTRACT-001; DeckApiTests.DeleteMetrics_ClearsSelectedOrAllResources | Metric data can be cleared for the selected resource or all resources. |
...
| METRIC-DIMENSIONS-001 | metrics | `/metrics` | metrics | covered | METRIC-DIMENSIONS-001; AOT-CONTRACT-001; DeckApiTests.GetMetrics_ReturnsSummariesAndSeries | Dimension filters and multiple attribute series are discoverable and selectable. |
| METRIC-HISTOGRAM-001 | metrics | `/metrics` | metrics | covered | METRIC-HISTOGRAM-001; HTTP-METRICS-001; AOT-CONTRACT-001; DeckApiTests.GetMetrics_ReturnsSummariesAndSeries; ChartDataCalculatorTests.TryCalculateHistogramAggregatePoint_ReturnsCumulativeDeltas | Histogram count and percentile views preserve aggregation semantics. |
| METRIC-EXEMPLARS-001 | metrics | `/metrics` | metrics | covered | METRIC-EXEMPLARS-001; AOT-CONTRACT-001; DeckApiTests.GetMetrics_ReturnsSummariesAndSeries | Metric exemplars expose values, timestamps, attributes, and related trace navigation. |
