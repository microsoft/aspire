...
test(`${features("TK-ICON-001")} resolves named icons, variants, and fallbacks`, async ({ page }, testInfo) => {
  ...
  await expect(regular).toHaveAttribute("fill", "none");
  ...
  expect(displayedMappings.map((mapping) => mapping.name)).toEqual(knownDashboardIconNames);
  for (const mapping of displayedMappings) {
    expect(mapping.regular).toBeTruthy();
    expect(mapping.filled).toBe(`${mapping.regular}Filled`);
  }
  ...
  await expect(catalog.getByText("Box resource fallback", { exact: true })).toBeVisible();
  await expect(catalog.getByText("LayoutGrid command fallback", { exact: true })).toBeVisible();
...
test(`${features("TK-STRUCTURED-FILTER-001")} composes structured telemetry filters`, async ({ page }) => {
  ...
  await page.getByRole("option", { name: "http.response.status_code" }).click();
...
test(`${features("TK-DIALOG-001", "TK-DRAWER-001")} exercises modal surfaces`, async ({ page }) => {
  ...
  await expect(toolkitDialog).toContainText("Reusable shadcn modal content.");
...
test(`${features("TK-SELECT-001", "TK-COMBOBOX-001", "TK-CHECKBOX-001", "TK-SWITCH-001", "TK-SECRET-001", "TK-SECRET-INPUT-001", "TK-COPY-001")} exercises input and sensitive-value controls`, async ({ page }) => {
  ...
  await expect(environment).toContainText("Development");
  await environment.click();
  await expect(page.getByText("Active", { exact: true })).toBeVisible();
  await expect(page.getByText("Archived", { exact: true })).toBeVisible();
  await expect(page.getByRole("option", { name: "Retired" })).toHaveAttribute("data-disabled");
...
test(`${features("TK-SHELL-001")} switches and persists a readable shadcn theme`, async ({ page }) => {
