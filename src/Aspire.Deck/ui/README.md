...
The UI is built on the reusable module in `src/toolkit`. The toolkit owns the Aspire-branded
shadcn/Radix primitives, Lucide icons, and domain-neutral controls such as buttons, badges, search,
...
- `DeckProvider` applies Deck's dark/light design tokens, density, typography, borders, and
  status colors to the owned shadcn component set.
...
therefore keep its domain logic local while sharing layout, focus behavior, owned components,
and visual tokens:
...
name to Lucide component mapping, regular/filled presentation, and both fallback contracts.

| Toolkit playground | Icon contract |
| --- | --- |
| ![Deck toolkit playground](docs/images/toolkit-playground.jpg) | ![Aspire to Lucide icon mapping](docs/images/toolkit-icons.jpg) |
...
src/
  ...
  toolkit/  Aspire-branded shadcn toolkit and reusable, domain-neutral UI primitives
...
- Toolkit controls own their shadcn/Radix component code and use Lucide icons while Deck CSS
  tokens define the product-specific color, spacing, and density.
