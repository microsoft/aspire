...
import { formatMetricValue, formatTimeWithOptionalDate } from "../lib/format";
          ...
          <tbody>
            {exemplars.map((exemplar, index) => (
              <tr key={`${exemplar.timestampMs}-${exemplar.traceId}-${index}`}>
                <td className="cell-mono">{formatTimeWithOptionalDate(new Date(exemplar.timestampMs), 3)}</td>
