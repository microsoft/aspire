...
import { displayUnit, formatMetricValue, formatTimeWithOptionalDate } from "../lib/format";
        ...
        <tbody>
          {series.timestampsMs.map((timestamp, index) => (
            <tr key={`${timestamp}-${index}`}>
              <td className="cell-mono">{formatTimeWithOptionalDate(new Date(timestamp))}</td>
