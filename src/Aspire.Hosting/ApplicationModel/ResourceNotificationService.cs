...
public class ResourceNotificationService : IDisposable
{
    ...
    private static bool CanTransitionToWaiting(string? state, bool allowNotStarted) =>
        state is null
        || (allowNotStarted && string.Equals(state, KnownResourceStates.NotStarted, StringComparisons.ResourceState))
        || string.Equals(state, KnownResourceStates.Starting, StringComparisons.ResourceState)
        || string.Equals(state, KnownResourceStates.Waiting, StringComparisons.ResourceState);
