...
internal sealed class ExecutableCreator : IObjectCreator<Executable, EmptyCreationContext>
{
    ...
    public async Task CreateObjectAsync(RenderedModelResource<Executable> er, EmptyCreationContext context, ILogger resourceLogger, IDcpObjectFactory factory, CancellationToken cancellationToken)
    {
        ...
        // Create IdeSession resources for browser-debuggable WASM clients served by this resource.
        // Each BrowserDebugAnnotation results in a separate IdeSession in "Initial" state;
        // the session transitions to "Running" when the user clicks "Debug in Browser".
        if (er.ModelResource.TryGetAnnotationsOfType<BrowserDebugAnnotation>(out var debugAnnotations))
        {
            foreach (var annotation in debugAnnotations)
            {
                var appUrl = ResolveDebugAppUrl(er.ModelResource, annotation);
                var sessionName = $"debug-{er.DcpResource.Metadata.Name}-{DcpNameGenerator.GetRandomNameSuffix()}";

                var spec2 = new IdeSessionSpec
                {
                    LaunchConfigurations =
                    [
                        new BrowserDebugLaunchConfiguration
                        {
                            ProjectPath = annotation.ClientProjectPath,
                            AppUrl = appUrl ?? string.Empty,
                        }
                    ],
                    DesiredState = IdeSessionState.Initial
                };

                var ideSession = IdeSession.Create(sessionName, spec2);

                try
                {
                    await factory.CreateDcpObjectsAsync([ideSession], cancellationToken).ConfigureAwait(false);

                    // Register the IdeSession as a tracked resource so it can be started
                    // via StartResourceAsync when the user clicks "Debug in Browser".
                    var ideSessionResource = new RenderedModelResource<IdeSession>(er.ModelResource, ideSession);
                    ideSessionResource.MarkInitialized();
                    _appResources.Add(ideSessionResource);

                    // Store the session name so the command handler can reference it later.
                    annotation.IdeSessionName = sessionName;
                }
                catch (k8s.Autorest.HttpOperationException ex) when (ex.Response?.StatusCode == System.Net.HttpStatusCode.NotFound)
                {
                    // DCP doesn't support IdeSession yet — degrade gracefully.
                    _logger.LogDebug(
                        "DCP does not support IdeSession resources. Browser debugging for '{ClientProject}' on resource '{ResourceName}' is unavailable.",
                        annotation.ClientProjectPath, er.ModelResource.Name);
                    continue;
                }
                ...
                {
                    _logger.LogWarning(
                        "Could not resolve app URL for WASM client '{ClientProject}' on resource '{ResourceName}'. " +
                        "The IdeSession was created but the IDE may not be able to navigate to the app.",
                        annotation.ClientProjectPath, er.ModelResource.Name);
                }
            }
        }
    ...
    /// <summary>
    /// Resolves the URL where the WASM application is served, using the annotation's custom resolver
    /// or falling back to the resource's first HTTPS/HTTP endpoint.
    /// </summary>
    private static string? ResolveDebugAppUrl(IResource resource, BrowserDebugAnnotation annotation)
    {
        // Use the first allocated HTTPS or HTTP endpoint on the resource.
        if (!resource.TryGetEndpoints(out var endpoints))
        {
            return null;
        }

        var endpoint = endpoints.FirstOrDefault(e => e.Name is "https" && e.AllocatedEndpoint is not null)
            ?? endpoints.FirstOrDefault(e => e.Name is "http" && e.AllocatedEndpoint is not null);
        ...
        var baseUrl = endpoint.AllocatedEndpoint.UriString;
        return annotation.RelativePath is { } path
            ? $"{baseUrl}/{path}/"
            : baseUrl;
    }
