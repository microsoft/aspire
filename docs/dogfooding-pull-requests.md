...
no `zap` stanza, because `~/.aspire/` is a shared prefix with the script
and PR installers and a brew-driven recursive delete would clobber state
...
NuGet hives under `~/.aspire/hives/` and any script or PR
