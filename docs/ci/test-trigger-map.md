...
- **Run-all fallback.** A changed `src/**` file that no Layer 2 rule matched,
  that is not ignored, and that is not under a project in `Aspire.slnx` forces
  the full matrix. A missed test is a silent regression; an extra run is just
  slower. Non-`src` leftovers are audit-only.
