...
16. [Resource Annotations](#resource-annotations-cross-language-resource-state)
...
## Resource Annotations (Cross-Language Resource State)

> **Terminology.** This spec uses "annotation" in two unrelated senses.
>
> - **Export annotation** — `[AspireExport]` / `AspireExport(...)` / a language-native decorator that marks a capability for cross-language projection. This is the meaning everywhere else in this document.
> - **Resource annotation** — named, serialized state attached to a resource in the AppHost application model. This is the subject of *this* section only.
>
> They are different mechanisms. This section always means the latter and qualifies it as "resource annotation".

An integration frequently needs to stash a piece of its own state on a resource during composition and read it back later — from a different capability, from a deferred callback, or from a **different integration in a different language** that extends the same resource. Resource annotations are the language-neutral primitive for that.

A resource annotation is:

- **Named** — keyed by a stable string ID that namespaces the value.
- **Single-valued** — one payload per ID, replace semantics. It is application-model state attached to a resource, not an append log, and not a persistent or distributed runtime store.
- **Serialized by value** — the storage and transport are a JSON string (`AtsAnnotation(annotationId, json)`), never a live object. That is what makes it cross-language: a value written in one language is read in another as long as both sides share the ID and the payload schema.

### Declaration

A resource annotation is declared as a stable ID paired with a payload DTO type — `AnnotationDefinition<TData>` in C#, the structurally-identical `AnnotationDefinition<T>` in TypeScript. The declaration carries no live state; it only binds an ID to a payload shape so typed accessors can serialize and deserialize the value without exposing the JSON-string storage that backs it.

### The primitive (exported) and typed sugar (per-language)

Three capabilities are projected onto every resource and form the language-neutral wire contract:

| Capability | Behavior |
|---|---|
| `withSerializedAnnotation(annotationId, json)` | Store or replace the payload. |
| `getSerializedAnnotation(annotationId): string` | Read the payload; throws when absent. |
| `hasSerializedAnnotation(annotationId): bool` | Presence check — lets one language read state that another may or may not have set. |
...
### Must be exported on the resource, not the builder

The exported capabilities target `IResource`, **not** `IResourceBuilder<T>`. Build-time guest calls carry resource-builder handles, but deferred run-time callbacks (an args callback, a Dockerfile-builder callback) are invoked with a bare resource handle. ATS has no overloading, and the dispatcher only converts a builder handle *down* to its resource (never a resource *up* to a builder), so a resource-targeted capability is the one shape reachable from both phases. Exporting the same method on both targets is invalid — one projection shadows the other. See [Handles](./polyglot-apphost.md#handles) for the full dispatch rule.

### Example

The Deno integration stores its computed app state during composition and reads it back from a deferred args callback to build the final `deno` command line:

```typescript
// One declaration shared by writers and readers.
const denoAppStateAnnotation = defineAnnotation<DenoAppState>("spike.deno/state");

// During composition — the store is a resource builder handle.
await setAnnotation(store, denoAppStateAnnotation, state);

// Later, inside a deferred run-time args callback — the store is a bare resource handle
// (generated resource and resource-builder wrappers both satisfy SerializedAnnotationStore).
const resource = await context.resource();
const state = await getAnnotation(resource, denoAppStateAnnotation);

const args = await context.args();
await args.clear();
for (const arg of getRunArgs(state)) {
    await args.add(arg); // e.g. ["task", "serve"] or ["run", "--allow-net", "main.ts"]
}
```

A C# integration could write the same `spike.deno/state` annotation with `WithAnnotation`, and the TypeScript callback above would read it unchanged — the value crosses the language boundary as camelCase JSON.
...
- The resource annotation primitive (`AtsAnnotation`, `AnnotationDefinition<T>`, the serialized `withSerializedAnnotation` / `getSerializedAnnotation` / `hasSerializedAnnotation` capabilities, and the typed `WithAnnotation` / `GetAnnotation` / `TryGetAnnotation` sugar) is internal spike surface today, not public product API.
...
- Decide whether to promote the resource annotation primitive from internal spike surface to public product API, and add a lower-level regression test that asserts the annotation capabilities project as resource-target only (no builder-target duplicate that could shadow them) and are invokable from both build-time builder handles and run-time resource handles.
