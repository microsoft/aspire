...
- The proposed immediate MVP fault surface remains open to review and change before approval. `http/v1` includes typed `latency`, `httpStatus`, and `rateLimit`. Azure resource profiles are Phase 0 contracts, not emulator-only shipping rows: `cosmos-gateway/v1` defines typed Gateway faults, `storage/v1` defines service-correct Blob, Queue, and Table faults, and `app-configuration/v1` defines typed latency and throttle. Once an HTTP(S) caller is pre-routed through YARP and the destination URI is known, emulator versus deployed Azure upstream is not a data-plane capability distinction. Each profile must use the same policy payload, typed fault semantics, and eligibility rules across every supported upstream location or the entire profile remains unavailable.
...
- `cosmos-gateway/v1` is one location-independent Phase 0 profile with protocol-correct 429 throttle, 449 Retry With, 412 precondition failed, 503 service unavailable, and latency behaviors. Eligibility requires a Gateway client with `LimitToEndpoint`, complete path capture, and the same semantics whether the upstream is the stable emulator or Azure. Experimental `RunAsPreviewEmulator` remains ineligible because its endpoint and trust lifecycle are not yet proven, not because it is an emulator. `resource` names an existing modeled account, database, or container resource, and optional `operations` selects `read`, `write`, or `query`; omitted means all operations in that resource scope except that `preconditionFailed` only fires on an ETag-conditional write.
...
Deployment location is not a profile or support boundary. Aspire infers a profile from modeled resource identity plus an eligible protocol, transport, and completely captured path; the same authored payload and typed wire behavior apply whether the upstream is an emulator or deployed Azure service. An emulator may be a conformance fixture, but it cannot define a smaller product tier. If automatic pre-start mediation, trust, authentication, authority, private-endpoint, warmed-client, and complete-path proofs do not pass for every supported location, the profile does not graduate. Applying a policy through CLI, Dashboard/MCP, or `Aspire.Hosting.Testing` is the explicit Run-mode consent before workload release; Publish remains byte-equivalent to the ordinary direct model.
...
- Whether App Configuration's complete HTTP(S) path remains mediated for warmed SDK clients and accepts the same protocol-correct 429 profile across the emulator fixture and Azure without client or AppHost changes.
...
1. When the profile policy is explicitly applied before its dependent workload is released, enumerate the selected structured references and allocate one stable localhost listener identity per destination/reference scope, including distinct caller identity when `fromResource` is supported.
...
4. Terminate client TLS with a locally trusted leaf for the stable listener identity, validate upstream TLS normally, and set the upstream HTTP Host header and TLS SNI from the stored resolved Azure authority so routing, signatures, and token audience remain service-correct.
...
This extension is not a chaos-specific AppHost API. It is a generic automatic native edge-synthesis and structured-reference-routing capability that chaos can consume. Applying the initial profile policy is the uniform Run-mode consent boundary and must happen before the selected workload constructs its client; the controller does not passively terminate TLS for any location before that consent. Publish and Deploy continue to resolve and emit the ordinary direct values, with no listener, trust material, policy, or rewritten reference.
...
`list-resources` and `describe-resource` resolve the app model and report the inferred graduated logical profile, eligible faults, their typed required and optional parameters, profile selectors, eligible `fromResource` callers, and actionable ineligibility reasons. These commands serialize the approved MVP support matrix directly, including ineligible Azure resources with no active profile or faults. Candidate profile, fault, and reference information may appear only as explicitly labeled assessment diagnostics; it is never treated as an inferred graduated profile, supported fault catalog, or input to policy validation. A developer does not need to guess resource names or understand CLR types, listeners, or address allocation. Each row shows:

Roadmap phase and candidate-contract details belong in the later graduation and ranked-assessment sections; `list-resources` reports current runtime eligibility and actionable reasons, not roadmap shorthand.
...
| Logical profile/version | Inferred graduated controller contract such as `http/v1`; an ungraduated contract may appear only as a clearly labeled candidate diagnostic and is not a CLR type |
...
| Eligible callers | Aspire resource names currently accepted by `fromResource`, grouped with the number of declared references they cover; candidate reference assessments are explicitly labeled and are not eligible callers |
...
| `carts` | `AzureCosmosDBContainerResource` | — (candidate: `cosmos-gateway/v1`) | `cosmos` -> `shop-db` -> `carts` | — (candidate preview: `latency`, `throttle`, `retryWith`, `preconditionFailed`, `serviceUnavailable`) | — (candidate references: `orders` (1)) | Not eligible — prove the modeled Gateway + `LimitToEndpoint` path, validated trust, automatic mediation, and exact typed behavior for both stable emulator and Azure upstreams |
| `storage` | `AzureStorageResource` | — (candidate: `storage/v1`) | — | — (candidate preview requires `service` and uses the selected Blob, Queue, or Table catalog) | — (candidate references: `worker` (1), `api` (1)) | Not eligible — prove every selected service path and typed behavior against both Azurite and Azure |
| `images` | `AzureBlobStorageContainerResource` | — (candidate: `storage/v1`) | `storage` -> `blobs` -> `images` | — (candidate preview: `latency`, `serverBusy`, conditional-write-only `preconditionFailed`) | — (candidate references: `api` (1)) | Not eligible — prove automatic mediation, URI-only container classification, conditional-write semantics, authentication, and private-endpoint coverage across supported Storage locations |
| `orders-queue` | `AzureQueueStorageQueueResource` | — (candidate: `storage/v1`) | `storage` -> `queues` -> `orders-queue` | — (candidate preview: `latency`, `serverBusy`) | — (candidate references: `worker` (1)) | Not eligible — prove every selected Queue path has identical payload and wire semantics across Azurite and Azure |
| `tables` | `AzureTableStorageResource` | — (candidate: `storage/v1`) | `storage` -> `tables` | — (candidate preview: `latency`, `serverBusy`, concrete-ETag `updateConditionNotSatisfied`) | — (candidate references: `api` (1)) | Not eligible — prove non-batch update/delete classification and exact responses across Azurite and Azure; `If-Match: *` and `$batch` remain excluded |
| `settings` | `AzureAppConfigurationResource` | — (candidate: `app-configuration/v1`) | — | — (candidate preview: `latency`, `throttle(retryAfter)`) | — (candidate references: `api` (1)) | Not eligible — prove automatic mediation, complete warmed-client path capture, auth, private endpoints, and fixed 429 behavior for both emulator and Azure upstreams |
...
For the proposed Cosmos profile, the same `resource` field may name an existing `AzureCosmosDBResource`, `AzureCosmosDBDatabaseResource`, or `AzureCosmosDBContainerResource`; see [How resource selection works](#how-resource-selection-works) for the account/database/container scoping table. No duplicate database or container string fields are added: `"resource": "carts"` selects the modeled container resource named `carts`, including its public parent and logical container identity. Storage similarly uses the modeled account, service, or child resource. A service or child infers Blob, Queue, or Table; the account requires one typed `service` value because the three endpoints have different wire contracts. Data Lake stays ineligible until both DFS and Blob paths pass as one complete profile; the absence of an emulator fixture is not a profile classification. Authors never select profile identifiers or upstream location; those remain derived implementation details.

`cosmos-gateway/v1` remains entirely in Phase 0 until one modeled-resource contract passes for stable emulator and Azure upstreams. Every eligible client must be proven before construction to use Gateway plus `LimitToEndpoint`; Direct/TCP (RNTBD), regional discovery, raw clients, component callbacks that bypass the listener, and consumers whose connection mode cannot be proven are ineligible regardless of upstream location and fail loudly rather than no-op. Stable `RunAsEmulator` is a useful fixture, but its disabled-validation connection string cannot establish the shipping trust contract. Experimental `RunAsPreviewEmulator` remains separately ineligible because its endpoint may transition from HTTP to HTTPS and its trust lifecycle is unproven. EF Core may use containers that are not modeled as `AzureCosmosDBContainerResource`; `list-resources` must warn about that gap, and container-scoped selection requires the AppHost to model the container with `AddContainer`.

### Stable startup and connection semantics

DCP resource-wide and per-reference proxy paths for ordinary resources are established eagerly before workload startup whether or not a policy is active. Every Azure resource profile uses one location-independent consent and capture contract: the initial policy must be applied before the selected workload is released, that explicit action allocates the path and rewrites every selected reference before client construction, and the upstream binds when its URI is available. This is identical for emulator and Azure upstreams. After the path exists, an empty policy set is pass-through. Applying and removing later policies never rewrites service-discovery values or restarts workloads.
...
- trusted localhost TLS termination, per-upstream-authority leaf identity, upstream HTTP Host and TLS SNI derived from the resolved Azure authority, and validated upstream TLS;
...
### Proposed profile contracts and MVP graduation matrix

The matrix below is a proposal and remains open to review and change before approval. `http/v1` is the only immediate shipping profile. Every Azure profile row is a Phase 0 contract and graduates as a whole only after its typed behavior and automatic mediation pass across every supported upstream location. Once a logical profile version is approved and shipped, discovery, validation, CLI prompting, Dashboard controls, MCP, and typed testing helpers all project the same enumerated schema regardless of emulator versus deployed upstream; location never adds, removes, or changes a fault. Later catalog changes use explicit profile versioning and compatibility review rather than generic fallback or location-specific variants.
...
No Azure resource-specific profile is in the immediate shipping matrix. The following location-independent contracts remain Phase 0:

| Profile contract | Modeled identity and eligible data plane | Required Phase 0 evidence before whole-profile graduation |
| --- | --- | --- |
...
| `cosmos-gateway/v1` | Modeled account/database/container paths proven as Gateway plus `LimitToEndpoint` | Identical payload and exact typed responses across stable emulator and Azure; regional and alternate endpoint exclusion; token audience; trust; complete warmed-client capture; Direct/RNTBD rejection everywhere |
| `app-configuration/v1` | Completely captured modeled App Configuration HTTP(S) path | Identical payload, fixed 429, and latency semantics across emulator and Azure; automatic edge synthesis; Entra and connection-string auth; private endpoints; complete warmed-client capture |
| `key-vault-https/v1` | Completely captured modeled Key Vault HTTPS path | Automatic pre-start native mediation, upstream HTTP Host and TLS SNI derived from the resolved vault authority, bearer-token audience, private endpoint, complete client-path, caller-isolation, retry, and decrypted-secret handling proofs |
| `search-https/v1` | Completely captured modeled AI Search HTTPS path | Automatic pre-start native mediation, upstream HTTP Host and TLS SNI derived from the resolved search authority, API-key and Entra auth, private endpoint, complete client-path, caller-isolation, latency, and fixed 503 proofs |

Every Azure profile remains unavailable until its whole location-independent row graduates through explicit matrix review. Data Lake, Azure OpenAI, Kusto, Service Bus, Event Hubs, Redis, SQL, PostgreSQL, SignalR, Web PubSub, and every other `Azure*Resource` outside the immediate `http/v1` row appear in discovery with no active profile and an actionable protocol, transport, path, or security reason. The controller never falls back from an unknown Azure resource type to `http/v1`, applies one Storage service's response template to another service, or uses upstream location to alter eligibility or schema.

Catalog membership is profile-specific and location-independent. Resolving ordinary `inventory` to `http/v1` permits only `latency`, `httpStatus`, and `rateLimit`. After graduation, resolving a modeled Cosmos resource on an eligible Gateway path permits the five Cosmos members above, and resolving a Storage account with `service: "blob"` or a modeled Blob child selects only the Blob members; Queue and Table resolve their own subsets inside the same `storage/v1` profile. Resolving an eligible App Configuration resource permits only `latency` and `throttle`. Emulator and Azure fixtures exercise the same payload and fixed response template; no fault, schema, response, selector, or profile version changes by upstream location.
...
| ---: | --- | --- | --- | --- | --- | --- |
| 1 | Storage: `AzureStorageResource`, `AzureBlobStorageResource`, `AzureBlobStorageContainerResource`, `AzureQueueStorageResource`, `AzureQueueStorageQueueResource`, and `AzureTableStorageResource` (`src/Aspire.Hosting.Azure.Storage`) | Azurite endpoints are modeled; Azure URI outputs are direct today. One proposed profile covers both | Distinct HTTP(S) services. Blob: latency, fixed 503 `ServerBusy`, and eligible conditional-write 412 `ConditionNotMet`. Queue: latency and fixed 503. Table: latency, fixed 503, and concrete-ETag update/delete 412. Batch and multipart operations reject without body parsing | Automatic mediation must cover every selected path before construction. Azure adds auth, authority, private-endpoint, signed-request, secondary-URI, and trust obligations; those are conformance requirements for the same profile | **Phase 0 then MVP as one profile.** No Azurite-only row. Graduate only after identical payload and typed semantics pass against Azurite and Azure across Entra, SharedKey, and SAS | Publish emits ordinary location-appropriate Storage references |
| 2 | App Configuration: `AzureAppConfigurationResource` (`src/Aspire.Hosting.Azure.AppConfiguration`) | The emulator endpoint is modeled; Azure output is direct today. One proposed HTTP(S) profile covers both | Latency and fixed 429 throttle with `retry-after-ms` and the documented [App Configuration throttling response](https://learn.microsoft.com/azure/azure-app-configuration/rest-api-throttling) | Prove automatic mediation, complete warmed SDK coverage, Entra and connection-string auth, authority, trust, private endpoints, and secret handling across supported locations | **Phase 0 then MVP as one profile.** No emulator-only row | Publish emits the ordinary location-appropriate reference |
| 3 | Cosmos Gateway: `AzureCosmosDBResource`, `AzureCosmosDBDatabaseResource`, and `AzureCosmosDBContainerResource` (`src/Aspire.Hosting.Azure.CosmosDB`) | Stable emulator and Azure account paths share one proposed Gateway contract; preview emulator has a distinct unproven lifecycle | Gateway HTTPS with latency, 429, 449, conditional-only 412, and 503 | Require Gateway plus `LimitToEndpoint` before construction, complete capture, validated trust, token audience, authority, warmed-client revisions, and regional/alternate endpoint exclusion on both stable emulator and Azure. Direct/RNTBD is ineligible everywhere | **Phase 0 then MVP as one profile.** No stable-emulator-only row; preview remains ineligible until its lifecycle passes the same contract | Publish emits the ordinary Cosmos reference |
...
`storage/v1` graduation requires one contract exercised against both Azurite and Azure. The minimum Azure fixture is a concrete Entra-authenticated `BlobServiceClient` proof:

1. With zero AppHost-source and application-source changes, an explicitly applied pre-workload `storage/v1` policy records the same consent used for every supported location, and caller-aware reference resolution substitutes the Azure Blob URI with a stable trusted localhost listener before the workload starts and before `BlobServiceClient` is constructed.
2. The listener binds its upstream only after provisioning resolves, validates the real certificate, and sets the upstream HTTP Host and TLS SNI from the stored resolved Storage authority while retaining the stable localhost listener identity. The client continues to request the fixed Azure Storage token audience rather than a localhost audience.
...
Before `storage/v1` graduates, run the same payload and typed assertions against Azurite and Azure, repeat the Azure proof with SharedKey and SAS, exercise each service-specific URI, prove secondary/retry endpoints cannot bypass the listener, and validate private-endpoint reachability. Security review must account for bearer tokens, SAS query values, SharedKey authorization, and signed request material becoming plaintext inside the local proxy after TLS termination. Failure in either location blocks the whole profile.
...
### Cosmos container profile contract (Phase 0 until graduation)
...
`cosmos-gateway/v1` targets modeled Cosmos resources whose clients are proven before construction to use Gateway plus `LimitToEndpoint`, independent of upstream location. Aspire's component currently defaults emulator clients to those settings before invoking the user callback, but callbacks and raw `CosmosClient` instances can bypass them. The controller must observe conformance and loudly reject every unproven mode. Stable `RunAsEmulator` and Azure Gateway are conformance fixtures for the same profile; the emulator's current `DisableServerCertificateValidation=True` connection string cannot satisfy the shipping trust contract.

The profile proof must preserve token audience, set the upstream HTTP Host and TLS SNI from the resolved account authority while retaining the stable listener address, validate trust, exclude regional discovery and failover bypass, and demonstrate live revisions on warmed clients against both stable emulator and Azure upstreams. Direct/RNTBD, raw clients, callbacks that restore Direct mode or disable `LimitToEndpoint`, and every unprovable mode are ineligible everywhere. Experimental `RunAsPreviewEmulator` remains ineligible because its endpoint may transition from HTTP to HTTPS and its trust lifecycle is unproven. EF Core container usage not represented by an `AzureCosmosDBContainerResource` is ineligible for container scope until the AppHost uses `AddContainer`.
...
### Storage profile contract (Phase 0 until graduation)
...
| Service | Modeled scope | Candidate typed faults | Fixed response behavior |
| --- | --- | --- | --- |
...
The controller rejects a policy before activation whenever its identifiers do not resolve cleanly. Profile-specific rows below apply only after that profile graduates; an ungraduated resource is rejected before its candidate selectors or faults are interpreted. The most important cases:
...
| `resource` names Storage before the location-independent `storage/v1` profile graduates, or has no completely mediated path for the selected service | Rejected as not eligible with actionable guidance identifying the missing automatic-mediation, location-parity, endpoint, or uncovered-reference proof |
...
| `resource` names App Configuration before the location-independent profile graduates, or its selected path is not completely mediated | Rejected as not eligible with actionable guidance identifying the automatic-mediation, location-parity, auth, private-endpoint, or complete-path gap |
| An Azure resource type is outside the immediate `http/v1` row or its profile has not graduated | Rejected with no active inferred profile or faults; a candidate contract may be named only as assessment context, for example: `vault (AzureKeyVaultResource) is not eligible; candidate key-vault-https/v1 requires automatic edge synthesis plus authority, auth, private-endpoint, complete-path, and secret-handling proof` |
| For a graduated profile, `fault.type` is not in the inferred resource catalog, or its member parameters are missing, mistyped, out of range, or unknown | Rejected with the inferred logical profile/version plus valid fault types, JSON types, constraints, and required/optional parameters, for example: `operations uses cosmos-gateway/v1; valid faults are latency(minimum: duration, maximum: duration), throttle(retryAfter: duration), retryWith(), preconditionFailed(), and serviceUnavailable()` |
...
Phase 1 accepts only the resource type, fault, parameter, and selector combinations in the shipping matrix. Initially that means `http/v1`; an Azure profile joins only after its whole location-independent contract graduates. `operations` is valid only for a graduated Cosmos profile and conditional faults require their standard request-metadata proofs. Storage, Cosmos, App Configuration, Key Vault, AI Search, Data Lake, and every other assessed profile remain unavailable until their named protocol, transport, path, security, and cross-location gates pass. No profile falls back to generic HTTP or exposes a smaller emulator-only catalog.
...
`describe-resource --resource storage` reports that `service` is required and lists `blob`, `queue`, and `table` with each service's exact fault union. Before `storage/v1` graduates, it reports the whole profile as unavailable with its cross-location mediation and conformance gaps; after graduation, it groups eligible callers by service without exposing upstream location as a support boundary. Data Lake remains separately ineligible because its DFS and Blob paths have not passed one complete profile contract.
...
- An Azure profile can carry production-capable bearer tokens, SAS values, SharedKey or API-key authorization, signed headers, and decrypted request content inside the local proxy process. Initial pre-workload policy application is the explicit consent boundary for the whole profile in Run mode; the controller must not passively pre-route or terminate traffic at any location before that boundary, and it does not expose a location-specific consent mode.
...
- If the host, container boundary, diagnostics pipeline, debugger/crash configuration, or memory-lifetime model cannot meet that posture at every supported location, the whole profile is rejected loudly. The controller must not silently fall back to direct traffic after reporting its policy as active.
...
- For a graduated real-resource profile, require an initial policy before releasing the selected workload, record that apply as explicit consent, substitute the caller's structured reference with the stable trusted listener before client construction, then bind the resolved upstream after provisioning and set its HTTP Host and TLS SNI from the stored Azure authority while preserving the token audience and stable client-facing listener identity.
...
### Phase 0 resource-profile candidates and explicitly deferred paths

- Automatic pre-start native mediation of Azure resource references is Phase 0 product work, not current DCP behavior. Known-URI YARP forwarding is established. `storage/v1`, `cosmos-gateway/v1`, `app-configuration/v1`, Key Vault, and AI Search may graduate only as location-independent profiles after automatic edge synthesis and their protocol, path, auth, security, and cross-location conformance gates pass.
- Cosmos Gateway faults remain Phase 0 until stable emulator and Azure fixtures prove the same latency, 429, 449, conditional 412, and 503 contract with Gateway plus `LimitToEndpoint`, validated trust, and complete path capture. Direct/RNTBD is ineligible everywhere.
- Storage faults remain Phase 0 until Azurite and Azure prove the same service-specific catalog, including exact response envelopes, conditional semantics, auth forms, private endpoints, and warmed-client coverage.
- App Configuration faults remain Phase 0 until emulator and Azure prove the same latency and fixed-429 contract with complete mediation and supported auth.
- Generic TCP faults.
...
| `cosmos-gateway/v1` traffic cannot be classified safely or TLS trust cannot be established cross-platform at every supported location | Apply any classification narrowing identically across locations under the stated rules; if mediation or trust fails at either location, block the whole profile; never ship a location-specific subset or silently no-op |
...
| An Azure profile has a modeled URI but DCP cannot automatically synthesize the pre-start edge, set the upstream HTTP Host and TLS SNI from the resolved Azure authority, preserve auth/audience, reach the private endpoint, cover every client path, or handle decrypted secrets safely | Keep the whole profile in its assessed phase with no active faults; known-URI YARP forwarding and existing trust delivery are precedent but not proof of automatic native mediation or service conformance, and the design never requires AppHost opt-in, client source changes, or disabled validation |
...
- Add and prove the generic Azure-resource seam across emulator and deployed upstreams: automatic pre-workload per-reference listener allocation, caller-aware structured reference rewrite, delayed upstream binding when needed, trusted localhost leaf identity, upstream HTTP Host and TLS SNI derived from the resolved authority, stable client-facing listener identity, validated upstream TLS, and direct publish output.
...
- Prove the same first-party Cosmos client-construction path forces or observes Gateway and `LimitToEndpoint` before construction for stable emulator and Azure upstreams. Preserve the token audience, derive the upstream HTTP Host and TLS SNI from the resolved account authority, and retain the stable client-facing listener identity; exclude regional discovery, alternate endpoints, Direct/RNTBD, raw clients, and unprovable callbacks; prove warmed-client live revisions and loud bypass rejection.
...
- Execute the minimum Entra-authenticated real `BlobServiceClient` PoC: automatic pre-start URI substitution to a stable trusted localhost listener, upstream HTTP Host and TLS SNI derived from the resolved Storage authority, fixed token audience, warmed client, live latency plus fixed 503 apply/remove without reconnect or restart, zero AppHost/application source changes, validated certificates, and byte-equivalent direct publish output.
- Repeat the Azure Storage proof with SharedKey, SAS, Queue, Table, secondary/retry URI bypass checks, and private endpoints; failure blocks `storage/v1` for Azurite too.
- Prove `app-configuration/v1` latency and 429 throttle with identical payload and response assertions against emulator and Azure, including fixed `retry-after-ms`, media type, problem body, warmed SDK retry behavior, caller isolation, auth forms, and private endpoints.
- Spike Key Vault, App Configuration, and AI Search as automatic native-mediation profiles. Known-URI YARP forwarding and existing trust delivery are reusable precedent, but each graduates only after automatic pre-start edge synthesis, upstream HTTP Host and TLS SNI derived from the resolved service authority, service auth forms, private endpoints, complete client paths, caller isolation, retry behavior, and decrypted-secret handling pass without AppHost code changes or location-specific schema.
...
- Prove Direct/RNTBD, ungraduated profiles, proxy-bypass, and otherwise unprovable connection modes reject eligibility loudly regardless of upstream location rather than silently no-op.
...
- Typed HTTP testing apply APIs with optional `fromResource`, returning `IAsyncDisposable` leases without authored profile, endpoint, or generic parameter-bag fields. Cosmos, Storage, App Configuration, and later profile helpers join only with their entire location-independent profile; Storage account scope then uses its typed `service` selector.
...
- No Azure profile is assumed in the initial Phase 1 shipping matrix. `cosmos-gateway/v1`, `storage/v1`, `app-configuration/v1`, and later profiles enter only through an explicitly reviewed whole-profile update after identical payload and typed semantics pass across every supported upstream location.
- A graduated Cosmos profile may expose typed `operations` (`read`, `write`, `query`; omitted means all) only if classification is proven without body parsing. `preconditionFailed` additionally requires ETag-conditional-write proof and never applies to unconditional requests.
- A graduated Storage profile exposes modeled account plus Blob service/container, Queue service/queue, and Table service selection with one service-correct catalog; account selection requires `service`.
...
| Azure-resource HTTPS | Known-URI YARP forwarding is established; automatic pre-start native mediation is a Phase 0 capability shared by emulator and deployed upstreams | Automatic stable-listener synthesis and caller-aware rewrite, delayed upstream binding, leaf identity and existing trust delivery, upstream HTTP Host and TLS SNI derived from resolved authority, stable client-facing identity, auth/audience preservation, private endpoints, complete paths, warmed clients, direct publish, and decrypted-secret security proof |
| Resource-specific MVP catalog | Ship `http/v1` initially; no Azure profile ships until its whole location-independent contract passes | Product review plus same-payload protocol conformance across every supported upstream location for every fixed wire shape |
| Cosmos profile | Keep one Phase 0 `cosmos-gateway/v1` contract for stable emulator and Azure Gateway; experimental preview lifecycle, Direct/RNTBD, raw clients, regional bypass, and unprovable modes remain outside | Resource hierarchy census, Gateway + `LimitToEndpoint` observation, token-audience and authority preservation, validated trust, identical 429/449/412/503 plus latency conformance across locations, warmed-client isolation, and loud rejection |
...
| Storage profile | Keep one Phase 0 service-specific `storage/v1` contract for Azurite and Azure Blob/Queue/Table | Prove identical payload and exact envelopes across locations, distinct service endpoints, conditional semantics, batch rejection, fixed Storage token audience, SharedKey/SAS, secondary bypass, private endpoints, warmed pools, direct publish, and sibling pass-through |
| App Configuration profile | Keep one Phase 0 `app-configuration/v1` contract for emulator and Azure HTTP(S) paths | Prove identical payload and fixed 429 semantics across locations, automatic edge synthesis, complete mediation, auth forms, authority, private endpoints, warmed SDK behavior, caller isolation, secret hygiene, and direct publish |
...
23. A graduated Cosmos policy names an existing modeled account, database, or container resource; no duplicate physical names or upstream-location selector appears in authored policy, and unmodeled EF Core containers produce a `list-resources` warning that directs the user to `AddContainer`.
24. Cosmos `operations` ships only if Gateway traffic proves profile-defined typed classification without body parsing against stable emulator and Azure fixtures. `retryWith` emits 449 Retry With with the `RetryWith` body, never a generic Conflict body. `preconditionFailed` ships only if standard request metadata proves an ETag-conditional write in both locations; an unprovable selector or member blocks the whole profile rather than being broadened.
25. `cosmos-gateway/v1` proofs demonstrate clients observed in Gateway mode with `LimitToEndpoint` and the same payload and complete catalog against stable emulator and Azure upstreams: bounded latency; protocol-correct 429 retry behavior; 449 Retry With with the `RetryWith` body; 412 precondition failed with substatus 0; and 503 service unavailable with substatus 0. Selected scope and operation behavior do not affect unrelated reads, unconditional creates, or sibling containers; warmed clients preserve cross-platform certificate validation. Experimental `RunAsPreviewEmulator` remains ineligible because its endpoint and trust lifecycle have not passed this contract. Direct/RNTBD remains ineligible everywhere.
26. Authored policy rejects `resourceType`, `profile`, and arbitrary parameter bags; a graduated logical profile/version appears only as derived list, describe, canonical result, dashboard, telemetry, and diagnostic metadata. Candidate identifiers may appear only as explicitly labeled assessment diagnostics and never drive policy validation.
27. Invalid resource/fault combinations for eligible resources report the inferred graduated profile/version, valid fault types, JSON types, constraints, and each member's required/optional parameters and selectors. An ineligible candidate resource reports no inferred profile or valid faults; any candidate contract or fault preview is diagnostic-only and clearly labeled. Interactive CLI and Dashboard resolve current eligibility before offering declared eligible callers and fault controls.
28. `list-resources` and `describe-resource` show currently eligible `fromResource` values and reference counts; candidate reference assessments, if shown, are explicitly diagnostic and cannot be selected. Callers with no declared edge are rejected, and one caller with multiple references is covered atomically.
...
30. The immediate shipping support matrix contains only `http/v1` for eligible ordinary non-Azure project/container destinations. `cosmos-gateway/v1`, `storage/v1`, `app-configuration/v1`, Key Vault, and AI Search remain whole-profile Phase 0 candidates until each passes automatic mediation and identical typed semantics across every supported upstream location. Every ungraduated Azure resource exposes no active inferred or fallback profile and no supported faults; any candidate preview is diagnostic-only, and the resource reports its protocol, transport, path, security, or cross-location proof gap.
...
42. `storage/v1` proves the same account/service/child scope, payload, and exact matrix responses against Azurite and Azure Blob, Queue, and Table. Blob 412 is limited to eligible conditional writes, conditional GET/HEAD 304 is untouched, Table 412 requires a non-batch update/delete with concrete ETag `If-Match`, `If-Match: *` is unconditional, Queue has no ETag fault, and batch/multipart operations reject without body parsing. Failure at either location blocks the profile.
43. `app-configuration/v1` proves the same latency and protocol-correct 429 throttle payload on warmed SDK connections against emulator and Azure upstreams, including supported auth and private-endpoint paths.
...
47. A generic zero-AppHost-source Azure-resource proof uses initial pre-workload policy application as explicit consent for emulator and deployed upstreams, automatically allocates a stable per-reference listener before releasing the workload, rewrites structured caller values, binds the upstream when resolved, terminates trusted localhost TLS, sets upstream HTTP Host and TLS SNI from the stored authority while retaining stable client-facing identity and token audience, applies later live revisions to warmed clients, and leaves Publish direct and byte-equivalent.
48. The `storage/v1` conformance suite uses the same payload and assertions against Azurite and Azure. Its Entra-authenticated `BlobServiceClient` fixture demonstrates pre-start substitution, validated certificates, warmed-client live latency and fixed 503 toggles without reconnect/restart, no AppHost/application source changes, and direct publish; SharedKey, SAS, Queue, Table, secondary bypass, and private endpoint proofs pass before the whole profile graduates.
49. `cosmos-gateway/v1` can graduate only when stable emulator and Azure clients are proven before construction to use Gateway plus `LimitToEndpoint`, with identical payload and typed-fault behavior, token audience, authority, trust, regional-bypass exclusion, warmed-client, and security proofs. Direct/RNTBD, raw clients, and unprovable modes remain ineligible everywhere. Cosmos SDK FaultInjection is documented only as a separate client-construction integration option.
...
52. Azure-resource TLS termination requires explicit pre-workload policy consent, reviewed process isolation, no credential/header/body logging, bounded plaintext memory lifetime, and loud rejection when the security posture cannot be met. Those obligations gate the whole profile rather than creating a deployed-resource tier.
53. No profile, schema, policy payload, fault member, selector, response template, or eligibility result differs because the upstream is an emulator or deployed service. An emulator may be a fixture, but any location-specific failure blocks the entire profile from graduation.
