...
| `carts` | `AzureCosmosDBContainerResource` | `cosmos-gateway/v1` | `cosmos` -> `shop-db` -> `carts` | Candidate `latency`, `throttle`, `retryWith`, `preconditionFailed`, `serviceUnavailable`; typed operation constraints apply | `orders` (1) | Phase 0 until the modeled Gateway + `LimitToEndpoint` path, validated trust, automatic mediation, and exact typed behavior pass for both stable emulator and Azure upstreams |
| `storage` | `AzureStorageResource` | `storage/v1` | — | Candidate profile requires `service`; faults are projected from the selected Blob, Queue, or Table catalog | `worker` (1), `api` (1) | Phase 0 until every selected service path and typed behavior pass against both Azurite and Azure |
| `images` | `AzureBlobStorageContainerResource` | `storage/v1` | `storage` -> `blobs` -> `images` | Candidate `latency`, `serverBusy`, eligible conditional-write-only `preconditionFailed` | `api` (1) | Phase 0 until automatic mediation, URI-only container classification, conditional-write semantics, authentication, and private-endpoint coverage pass across supported Storage locations |
| `orders-queue` | `AzureQueueStorageQueueResource` | `storage/v1` | `storage` -> `queues` -> `orders-queue` | Candidate `latency`, `serverBusy` | `worker` (1) | Phase 0 until every selected Queue path is mediated with identical payload and wire semantics across Azurite and Azure |
| `tables` | `AzureTableStorageResource` | `storage/v1` | `storage` -> `tables` | Candidate `latency`, `serverBusy`, concrete-ETag `updateConditionNotSatisfied` | `api` (1) | Phase 0 until non-batch update/delete classification and exact responses pass across Azurite and Azure; `If-Match: *` and `$batch` are excluded |
| `settings` | `AzureAppConfigurationResource` | `app-configuration/v1` | — | Candidate `latency`, `throttle(retryAfter)` | `api` (1) | Phase 0 until automatic mediation, complete warmed-client path capture, auth, private endpoint, and fixed 429 behavior pass for both emulator and Azure upstreams |
| `vault` | `AzureKeyVaultResource` | — | — | — | — | Phase 0 candidate: current DCP does not automatically synthesize a listener from the resolved vault URI; native mediation, existing trust-delivery extension, authority/auth/private-endpoint, complete-path, and secret-handling proofs remain |
...
| `resource` names Storage before the location-independent `storage/v1` profile graduates, or has no completely mediated path for the selected service | Rejected with `storage/v1` Phase 0 guidance that identifies the missing automatic-mediation, location-parity, endpoint, or uncovered-reference proof |
...
| `resource` names App Configuration before the location-independent profile graduates, or its selected path is not completely mediated | Rejected with `app-configuration/v1` Phase 0 guidance identifying the automatic-mediation, location-parity, auth, private-endpoint, or complete-path gap |
| An Azure resource type is outside the immediate `http/v1` row or its profile has not graduated | Rejected with no active inferred profile or faults and its assessment phase, for example: `vault (AzureKeyVaultResource) requires the key-vault-https/v1 automatic edge-synthesis, authority, auth, private-endpoint, complete-path, and secret-handling proof` |
...
30. The immediate shipping support matrix contains only `http/v1` for eligible ordinary non-Azure project/container destinations. `cosmos-gateway/v1`, `storage/v1`, `app-configuration/v1`, Key Vault, and AI Search remain whole-profile Phase 0 candidates until each passes automatic mediation and identical typed semantics across every supported upstream location. Every ungraduated Azure resource exposes no fallback profile or faults and reports its protocol, transport, path, security, or cross-location proof gap.
