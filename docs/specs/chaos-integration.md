...
For the Azure MVP, admit only paths that the existing Run-mode model already makes credibly interceptable without chaos-specific AppHost setup: the App Configuration emulator, the Blob, Queue, and Table endpoints of Azurite, and the stable HTTPS Cosmos `RunAsEmulator` path, each behind its named proof gates. Experimental `RunAsPreviewEmulator` is outside the immediate matrix because its endpoint starts as HTTP, may switch to HTTPS after developer-certificate setup, and follows a different endpoint and trust lifecycle. Do not treat a modeled Azure URI as proof that DCP can dynamically intercept a real service. Real-resource HTTPS, private endpoints, token audiences, process and container trust stores, streaming transports, and protocol-specific connection pools must be proven before their typed profiles graduate. Applying a policy through CLI, Dashboard/MCP, or `Aspire.Hosting.Testing` is the explicit Run-mode consent; there is no earlier per-resource opt-in.
...
- Whether Aspire-managed double-leg TLS interception, or an explicitly approved equivalent, can replace stable Cosmos `RunAsEmulator`'s current disabled-validation baseline with validated trust across Windows, Linux, macOS, and supported containers.
...
| 3 | Cosmos: `AzureCosmosDBResource`, `AzureCosmosDBDatabaseResource`, and `AzureCosmosDBContainerResource` (`src/Aspire.Hosting.Azure.CosmosDB`) | Stable `RunAsEmulator` only, with observed Gateway-mode clients; experimental `RunAsPreviewEmulator` with its initial-HTTP, potentially HTTPS endpoint lifecycle, real accounts, and Direct/RNTBD are excluded | Gateway HTTPS. Preserve latency, 429 throttle, 449 concurrency conflict, conditional-only 412 precondition failed, and 503 service unavailable | Credible only for the stable endpoint after replacing its current disabled-validation connection-string baseline with cross-platform validated trust or an explicitly approved equivalent, plus URI/method/header classification. Component defaults are not proof: user callbacks can override Gateway/`LimitToEndpoint`, and raw clients can bypass them | **Immediate MVP, release-gated.** Preserve all existing scope, trust, no-body-parsing, and loud rejection of unobserved or bypassing client modes | Publish emits the normal Cosmos model and direct references only |
...
| The Cosmos resource uses experimental `RunAsPreviewEmulator`, or targets a real account | Rejected as outside the immediate stable `RunAsEmulator` matrix; preview's initially HTTP endpoint may transition to HTTPS after developer-certificate setup, but its distinct endpoint and trust lifecycle require separate proof |
...
| Cosmos profile | Phase 1 modeled stable `RunAsEmulator` Gateway HTTPS only, release-gated on replacing its current disabled-validation baseline with validated trust or an explicitly approved equivalent; experimental `RunAsPreviewEmulator` is outside the immediate matrix because its endpoint starts as HTTP, may transition to HTTPS after developer-certificate setup, and has a different endpoint and trust lifecycle; keep the typed profile in Aspire and DCP generic | Resource hierarchy census, observed Gateway-mode clients, validated-trust interception, 429/449/412/503 plus latency conformance, warmed-client isolation, and loud rejection of bypass modes |
...
30. The shipping support matrix contains `http/v1` for eligible ordinary non-Azure project/container destinations, `cosmos-gateway/v1` for modeled stable `RunAsEmulator` Cosmos account/database/container resources with observed Gateway-mode clients and the validated-trust release gate satisfied, one `storage/v1` profile with service-specific Azurite Blob/Queue/Table unions, and `app-configuration/v1` for the local emulator. Experimental `RunAsPreviewEmulator`, whose endpoint starts as HTTP and may transition to HTTPS after developer-certificate setup, and every other Azure resource type expose no fallback profile or faults and report their assessment phase and proof gap.
