...
## Scenario: I Need Browser For A JavaScript Frontend

Use this when the AppHost is `apphost.ts` and the frontend is a Vite app or another JavaScript app. Browser is optimized for frontend applications, so attach it to the browser-served frontend resource rather than backend services.

```bash
aspire add browsers
```

For Vite frontends, attach browser to the Vite resource:
...
For generic JavaScript frontends that run from a package.json script, attach browser to the JavaScript app resource:
...
- After editing, restart with `aspire start --isolated` and use `aspire describe --format Json` to find the generated `<frontend>-browser` resource.
