...
public class TrayLaunchCommandTests
{
    ...
    public void LaunchServicesGetsExactBundleAndCliPathsWithoutShellQuoting()
    {
        ...
        var options = SpikeOptions.Parse(["--cli", bundle.CliPath, "--bundle-root", bundle.Root]);
    ...
    {
        using var bundle = new TestTrayBundle();
        Assert.Equal(new(bundle.CliPath, null, null), SpikeOptions.Parse(["--cli", bundle.CliPath]));
    }
    ...
    {
        using var bundle = new TestTrayBundle();
        Assert.Throws<ArgumentException>(() => SpikeOptions.Parse(["--cli", bundle.CliPath, option, value]));
    }
    ...
    {
        using var bundle = new TestTrayBundle();
        Assert.Throws<ArgumentException>(() => SpikeOptions.Parse([
