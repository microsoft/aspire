...
A macOS-first experiment in a **C# NativeAOT** companion for the Aspire CLI.
...
`~/Library/Application Support/Aspire/TraySpike/control-v1.sock`.
...
Quit any older prototype using the previous activation protocol before trying
this build.
...
```sh
./restore.sh -projects "$PWD/tools/Aspire.Tray.Spike/Mac/Aspire.Tray.Spike.Mac.csproj"
```

Publish for the current machine, or pass `osx-arm64` or `osx-x64` explicitly:

```sh
bash tools/Aspire.Tray.Spike/Mac/publish.sh
```
...
"artifacts/bin/Aspire.Tray/Release/net10.0/osx-arm64/app/Aspire Tray.app/Contents/MacOS/aspire-tray" \
...
for this spike); normal launches retain the user's placement. This recovery is
...
dotnet test --project tools/Aspire.Tray.Spike/Tests/Aspire.Tray.Spike.Tests.csproj \
    --no-launch-profile -- \
...
for this spike. The Windows scaffold shares the typed backend/controller, but its
