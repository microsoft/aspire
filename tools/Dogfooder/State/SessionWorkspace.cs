...
internal sealed class SessionWorkspace
{
    ...
    /// <summary>
    /// Writes a <c>NuGet.config</c> at the workspace root that wires every
    /// <c>dotnet</c> / <c>aspire</c> command launched from this workspace
    /// to the local proxy at <paramref name="serviceIndexUrl"/>, with a
    /// <c>&lt;clear /&gt;</c> on <c>packageSources</c> so the user's
    /// machine-wide <c>nuget.org</c> source can't sneak back in.
    /// </summary>
    /// <remarks>
    /// NuGet walks up from <c>$CWD</c> aggregating every <c>NuGet.config</c>
    /// it finds; placing one at the workspace root (the terminal's CWD)
    /// means the CLI's restore, <c>dotnet add package</c>, <c>dotnet
    /// package search</c>, and even ad-hoc <c>dotnet new</c> templates all
    /// route through the proxy. Without this file the user's
    /// <c>~/.nuget/NuGet/NuGet.Config</c> wins and the proxy is silently
    /// bypassed, defeating the whole point of <c>UseLocalNuGetProxy</c>.
    /// See https://learn.microsoft.com/nuget/consume-packages/configuring-nuget-behavior.
    /// Also pins the global packages folder to the per-session cache so
    /// nothing escapes into the user's <c>~/.nuget/packages</c>.
    /// </remarks>
    public string WriteNuGetConfig(string serviceIndexUrl)
    {
        var path = Path.Combine(Root, "NuGet.config");
        // Built by hand so we can guarantee the exact &lt;clear /&gt; +
        // single-source shape. An XML serialiser would over-decorate.
        var xml = $"""
            <?xml version="1.0" encoding="utf-8"?>
            <configuration>
              <!--
                Auto-generated by aspire-dogfooder. Routes every dotnet/aspire
                command run from this directory through the local NuGet proxy.
                Delete this file (or `cd` out of the directory) to fall back
                to the user's normal NuGet configuration.
              -->
              <packageSources>
                <clear />
                <add key="aspire-dogfood-proxy" value="{System.Security.SecurityElement.Escape(serviceIndexUrl)}" />
              </packageSources>
              <packageSourceMapping>
                <clear />
                <packageSource key="aspire-dogfood-proxy">
                  <package pattern="*" />
                </packageSource>
              </packageSourceMapping>
              <config>
                <add key="globalPackagesFolder" value="{System.Security.SecurityElement.Escape(NuGetCacheDir)}" />
              </config>
            </configuration>
            """;
